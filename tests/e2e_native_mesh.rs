...
//! - Quasar: PQ security overlay with dual BLS + Corona certs
//!
...
/// ML-DSA-87 keypair (FIPS 204) - Digital Signature Algorithm (Corona)
#[derive(Clone)]
pub struct CoronaKeyPair {
...
/// Validator identity: Node key (ed25519) + BLS key + Corona key
#[derive(Clone)]
pub struct ValidatorIdentity {
    pub id: String,
    pub kem: KemKeyPair,
    pub corona: CoronaKeyPair,  // PQ signatures (Quasar Phase II)
...
impl ValidatorIdentity {
    ...
    pub fn public_key_hash(&self) -> [u8; 32] {
        ...
        data.extend_from_slice(&self.corona.verifying_key);
...
/// Dual certificate: BLS aggregate (classical) + Corona (PQ)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CertBundle {
    pub bls_aggregate: Vec<u8>,      // BLS aggregate signature (96 bytes)
    pub corona_cert: Vec<u8>,      // ML-DSA threshold signature (~3KB)
    pub epoch: u64,                   // Corona epoch number
    pub validator_set: Vec<String>,   // Attesting validators
    pub bls_signers: Vec<String>,     // BLS signers
    pub corona_signers: Vec<String>,// Corona signers
}

impl CertBundle {
    /// Both BLS AND Corona must be valid for quantum finality
    pub fn is_quantum_final(&self) -> bool {
        !self.bls_aggregate.is_empty() && !self.corona_cert.is_empty()
    }
}
...
pub struct ZapMessage {
    ...
    pub trace_id: u64,
    pub signature: Option<[u8; 4627]>,  // Corona signature
}
...
impl ZapMessage {
    ...
    pub fn sign(&mut self, identity: &ValidatorIdentity) {
        ...
        self.signature = Some(identity.corona.sign(&data));
    }
...
impl NativeMesh {
    ...
    pub async fn register(&self, identity: ValidatorIdentity) -> mpsc::Receiver<ZapMessage> {
        ...
        println!("  [+] Validator '{}' registered (BLS + Corona + ML-KEM-1024)", identity.id);
        rx
    }
...
mod tests {
    ...
    async fn test_native_pq_handshake() {
        ...
        agent_a.send(&bob_id, ZapContent::Text("Hello from Alice with Corona signature!".to_string())).await.unwrap();
        tokio::time::sleep(Duration::from_millis(10)).await;

        let msg = agent_b.recv(1000).await;
        assert!(msg.is_some());
        let msg = msg.unwrap();
        assert!(msg.signature.is_some(), "Message should be Corona-signed");
        println!("  [ok] Bob received Corona-signed message (sig: {:x?}...)", &msg.signature.unwrap()[..16]);
    ...
    /// Focus confidence, dual BLS + Corona certificate generation.
    ///
    ...
    async fn test_quasar_consensus() {
        ...
        // Phase II (Quasar): Dual BLS + Corona certificate
        // ================================================================
        ...
        println!("\n  ---- Quasar Phase II: Dual Certificate (BLS + Corona) ----\n");
        ...
        // Corona threshold: each validator signs with ML-DSA-87
        let cert_data = format!("{final_round}:{}", hex::encode(winning_hash));
        let mut corona_cert = Vec::new();
        let mut corona_signers = Vec::new();
        for v in &validators {
            let sig = v.identity.corona.sign(cert_data.as_bytes());
            corona_cert.extend_from_slice(&sig[..64]); // Truncated for display
            corona_signers.push(v.id().to_string());
        }
        println!("  Corona cert: {:x?}... ({} signers, {}B)", &corona_cert[..16], corona_signers.len(), corona_cert.len());
        ...
        assert!(cert.is_quantum_final(), "Both BLS and Corona must be present");
        ...
        println!("    corona_cert:  {:x?}... ({}B)", &cert.corona_cert[..16], cert.corona_cert.len());
        ...
        println!();
        println!("  Crypto: BLS12-381 (Phase I) + ML-DSA-87/Corona (Phase II)");
        ...
        assert!(cert.is_quantum_final(), "CertBundle needs both BLS and Corona");
