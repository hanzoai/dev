...
def test_cli_import_with_broken_third_party_runtime():
    ...
    modules_to_clear = [
        k for k in sys.modules.keys() if 'ide' in k or 'third_party' in k
    ]
...
def test_runtime_import_robustness():
    ...
    modules_to_clear = [k for k in sys.modules.keys() if 'ide.runtime' in k]
    for module in modules_to_clear:
        del sys.modules[module]

    # Import the runtime module - should succeed even with broken third-party runtimes
    try:
        import ide.runtime  # noqa: F401
...
def test_get_runtime_cls_works():
    ...
    docker_runtime = ide.runtime.get_runtime_cls('docker')
    assert docker_runtime is not None

    local_runtime = ide.runtime.get_runtime_cls('local')
...
def test_runtime_exception_handling():
    ...
    assert hasattr(ide.runtime, 'get_runtime_cls')
    assert hasattr(ide.runtime, '_THIRD_PARTY_RUNTIME_CLASSES')
...
def test_runtime_import_exception_handling_behavior():
    ...
    # Add our test handler to the IDE logger
