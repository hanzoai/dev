...
from ide.integrations.bitbucket.bitbucket_service import BitBucketService
from ide.integrations.provider import ProviderToken, ProviderType
from ide.integrations.service_types import ProviderType as ServiceProviderType
from ide.integrations.service_types import Repository
from ide.integrations.utils import validate_provider_token
from ide.resolver.interfaces.bitbucket import BitbucketIssueHandler
from ide.resolver.interfaces.issue import Issue
from ide.resolver.interfaces.issue_definitions import ServiceContextIssue
from ide.resolver.send_pull_request import send_pull_request
from ide.runtime.base import Runtime
from ide.server.routes.secrets import check_provider_tokens
from ide.server.settings import POSTProviderModel
from ide.server.types import AppMode
...
@patch('ide.resolver.send_pull_request.ServiceContextIssue')
@patch('ide.resolver.send_pull_request.BitbucketIssueHandler')
...
def test_send_pull_request_bitbucket(
    ...
    mock_service.get_branch_name.return_value = 'ide-fix-123'
...
class TestBitbucketProviderDomain(unittest.TestCase):
    ...
    @patch('ide.runtime.base.Runtime.__abstractmethods__', set())
    @patch('ide.runtime.utils.edit.FileEditRuntimeMixin.__init__', return_value=None)
    @patch('ide.runtime.base.ProviderHandler')
    @pytest.mark.asyncio
...
async def test_validate_provider_token_with_bitbucket_token():
    ...
    with (
        patch('ide.integrations.utils.GitHubService') as mock_github_service,
        patch('ide.integrations.utils.GitLabService') as mock_gitlab_service,
        patch('ide.integrations.utils.BitBucketService') as mock_bitbucket_service,
    ):
        # Set up the mocks
...
async def test_check_provider_tokens_with_only_bitbucket():
    ...
    with patch('ide.server.routes.secrets.validate_provider_token', mock_validate):
...
@patch('ide.core.setup.call_async_from_sync')
@patch('ide.core.setup.get_file_store')
@patch('ide.core.setup.EventStream')
def test_initialize_repository_for_runtime_with_bitbucket_token(
    ...
    from ide.integrations.provider import ProviderType
    ...
    with patch.dict(os.environ, {'BITBUCKET_TOKEN': 'username:app_password'}):
        result = initialize_repository_for_runtime(
            runtime=mock_runtime, selected_repository='hanzoai/test-repo'
        )
    ...
    assert args[3] == 'hanzoai/test-repo'  # selected_repository
