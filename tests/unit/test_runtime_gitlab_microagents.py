...
from ide.microagent.microagent import (
    RepoMicroagent,
)
from ide.runtime.base import Runtime
...
class MockRuntime(Runtime):
    ...
    def run_action(self, action):
        ...
        from ide.events.observation import CmdOutputObservation

        return CmdOutputObservation(content='', exit_code=0)

    def read(self, action):
        """Mock read method."""
        from ide.events.observation import ErrorObservation
    ...
    def _load_microagents_from_directory(self, directory: Path, source: str):
        ...
        for md_file in directory.rglob('*.md'):
            ...
            from ide.microagent.types import MicroagentMetadata, MicroagentType
    ...
    def run_ipython(self, action):
        from ide.events.observation import IPythonRunCellObservation

        return IPythonRunCellObservation(content='', code='')

    def edit(self, action):
        from ide.events.observation import FileEditObservation

        return FileEditObservation(content='', path='')

    def browse(self, action):
        from ide.events.observation import BrowserObservation
    ...
    def write(self, action):
        from ide.events.observation import FileWriteObservation
    ...
    def call_tool_mcp(self, action):
        from ide.events.observation import MCPObservation

        return MCPObservation(content='', tool='', result='')


def create_test_microagents(base_dir: Path, config_dir_name: str = '.ide'):
...
def test_is_gitlab_repository_github(temp_workspace):
    ...
    with patch('ide.runtime.base.ProviderHandler') as mock_handler_class:
        mock_handler = MagicMock()
        mock_handler_class.return_value = mock_handler

        with patch('ide.runtime.base.call_async_from_sync') as mock_async:
...
def test_get_microagents_from_org_or_user_github(temp_workspace):
    """Test that GitHub repositories only try .ide directory."""
    runtime = MockRuntime(temp_workspace)
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=False):
        ...
        with patch('ide.runtime.base.call_async_from_sync') as mock_async:
            ...
            # Should only try .ide, not ide-config
            assert len(result) == 0
            # Check that only one attempt was made (for .ide)
            assert mock_async.call_count == 1


def test_get_microagents_from_org_or_user_gitlab_success_with_config(temp_workspace):
    """Test that GitLab repositories use ide-config and succeed."""
    runtime = MockRuntime(temp_workspace)

    # Create a mock org directory with microagents
    org_dir = temp_workspace / 'org_ide_owner'
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        # Mock successful cloning for ide-config
        with patch('ide.runtime.base.call_async_from_sync') as mock_async:
            mock_async.return_value = 'https://gitlab.com/owner/ide-config.git'

            result = runtime.get_microagents_from_org_or_user('gitlab.com/owner/repo')

            # Should succeed with ide-config
            assert len(result) >= 0  # May be empty if no microagents found
            # Should only try once for ide-config
            assert mock_async.call_count == 1


def test_get_microagents_from_org_or_user_gitlab_failure(temp_workspace):
    """Test that GitLab repositories handle failure gracefully when ide-config doesn't exist."""
    runtime = MockRuntime(temp_workspace)

    # Mock the provider detection to return GitLab
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        # Mock the _get_authenticated_git_url to fail for ide-config
        with patch('ide.runtime.base.call_async_from_sync') as mock_async:
            mock_async.side_effect = Exception('ide-config not found')
...
def test_get_microagents_from_selected_repo_gitlab_uses_ide(temp_workspace):
    """Test that GitLab repositories use .ide directory for repository-specific microagents."""
    runtime = MockRuntime(temp_workspace)
    ...
    # Create microagents in .ide directory
    create_test_microagents(repo_dir, '.ide')
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=True):
        ...
        with patch.object(runtime, 'get_microagents_from_org_or_user', return_value=[]):
            ...
            # Should find microagents from .ide directory
            # The exact assertion depends on the mock implementation
            # At minimum, it should not raise an exception
            assert isinstance(result, list)


def test_get_microagents_from_selected_repo_github_only_ide(temp_workspace):
    """Test that GitHub repositories only check .ide directory."""
    runtime = MockRuntime(temp_workspace)
    ...
    create_test_microagents(repo_dir, 'ide-config')
    create_test_microagents(repo_dir, '.ide')
    ...
    with patch.object(runtime, '_is_gitlab_repository', return_value=False):
        ...
        with patch.object(runtime, 'get_microagents_from_org_or_user', return_value=[]):
            ...
            # Should only check .ide directory, not ide-config
            assert isinstance(result, list)
