...
from ide.resolver.interfaces.gitlab import GitlabIssueHandler, GitlabPRHandler
from ide.resolver.interfaces.issue import Issue, ReviewThread
from ide.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from ide.resolver.issue_resolver import (
    IssueResolver,
)
from ide.resolver.resolver_output import ResolverOutput
...
def mock_gitlab_token():
    ...
    """
    with patch(
        'ide.resolver.issue_resolver.identify_token',
...
async def test_process_issue(
    ...
    with (
        patch('ide.resolver.issue_resolver.create_runtime', mock_create_runtime),
        patch('ide.resolver.issue_resolver.run_controller', mock_run_controller),
        patch.object(
        ...
        patch(
            'ide.resolver.issue_resolver.SandboxConfig', return_value=MagicMock()
        ),
        patch(
            'ide.resolver.issue_resolver.IDEConfig',
...
def test_file_instruction():
    ...
    # load prompt from ide/resolver/prompts/resolve/basic.jinja
    with open('ide/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()

    with open(
        'ide/resolver/prompts/resolve/basic-conversation-instructions.jinja', 'r'
    ) as f:
...
def test_file_instruction_with_repo_instruction():
    ...
    # load repo instruction from ide/resolver/prompts/repo_instructions/all-hands-ai___openhands-resolver.txt
    with open(
        'ide/resolver/prompts/repo_instructions/all-hands-ai___openhands-resolver.txt',
        'r',
    ) as f:
