...
@patch('dev.resolver.interfaces.gitlab.GitlabIssueHandler.reply_to_comment')
@patch('httpx.post')
...
def test_send_pull_request(
    ...
    if pr_type == 'branch':
        assert (
            result
            == 'https://gitlab.com/test-owner/test-repo/-/compare/main...dev-fix-issue-42'
        )
        ...
        assert post_data['source_branch'] == 'dev-fix-issue-42'
...
def test_send_pull_request_branch_naming(
    ...
    assert (
        result
        == 'https://gitlab.com/test-owner/test-repo/-/compare/main...dev-fix-issue-42-try3'
    )
