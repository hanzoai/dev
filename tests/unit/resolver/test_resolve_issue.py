...
from ide.core.config import IDEConfig, SandboxConfig
...
def assert_sandbox_config(
    ...
    runtime_container_image='ghcr.io/hanzoai/runtime:mock-nikolaik',  # Default to mock version
...
def test_setup_sandbox_config_default():
    ...
    with mock.patch('ide.__version__', 'mock'):
        ...
        assert_sandbox_config(
            ide_config.sandbox,
            runtime_container_image='ghcr.io/hanzoai/runtime:mock-nikolaik',
        )
...
def test_setup_sandbox_config_runtime_only():
    ...
    assert_sandbox_config(
        ide_config.sandbox, runtime_container_image=runtime_image
    )
...
def test_setup_sandbox_config_experimental():
    ...
    with mock.patch('ide.__version__', 'mock'):
        ...
        assert_sandbox_config(ide_config.sandbox, runtime_container_image=None)


@mock.patch('ide.resolver.issue_resolver.os.getuid', return_value=0)
@mock.patch('ide.resolver.issue_resolver.get_unique_uid', return_value=1001)
def test_setup_sandbox_config_gitlab_ci(mock_get_unique_uid, mock_getuid):
    ...
    with mock.patch('ide.__version__', 'mock'):
        with mock.patch.object(IssueResolver, 'GITLAB_CI', True):
            ...
            assert_sandbox_config(
                ide_config.sandbox, local_runtime_url='http://localhost'
            )


@mock.patch('ide.resolver.issue_resolver.os.getuid', return_value=1000)
...
@mock.patch('ide.events.observation.CmdOutputObservation')
@mock.patch('ide.runtime.base.Runtime')
