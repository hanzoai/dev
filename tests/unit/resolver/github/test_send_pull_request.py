...
from ide.core.config import LLMConfig
from ide.integrations.service_types import ProviderType
from ide.resolver.interfaces.github import GithubIssueHandler
from ide.resolver.interfaces.issue import ReviewThread
from ide.resolver.resolver_output import Issue, ResolverOutput
from ide.resolver.send_pull_request import (
    apply_patch,
    initialize_repo,
...
@patch('ide.resolver.interfaces.github.GithubIssueHandler.reply_to_comment')
@patch('httpx.post')
@patch('subprocess.run')
@patch('ide.resolver.send_pull_request.LLM')
...
def test_send_pull_request(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'ide-fix-issue-42'],
        capture_output=True,
        text=True,
    )
    ...
    if pr_type == 'branch':
        assert (
            result
            == 'https://github.com/test-owner/test-repo/compare/ide-fix-issue-42?expand=1'
        )
        ...
        assert post_data['head'] == 'ide-fix-issue-42'
...
def test_send_pull_request_target_branch_with_fork(
    ...
    assert (
        post_data['head'] == 'fork-owner:ide-fix-issue-42'
...
@patch('ide.resolver.send_pull_request.initialize_repo')
@patch('ide.resolver.send_pull_request.apply_patch')
@patch('ide.resolver.send_pull_request.update_existing_pull_request')
@patch('ide.resolver.send_pull_request.make_commit')
...
@patch('ide.resolver.send_pull_request.initialize_repo')
@patch('ide.resolver.send_pull_request.apply_patch')
@patch('ide.resolver.send_pull_request.send_pull_request')
@patch('ide.resolver.send_pull_request.make_commit')
...
def test_send_pull_request_branch_naming(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'ide-fix-issue-42-try3'],
        capture_output=True,
        text=True,
    )
    assert push_call == call(
    ...
    assert (
        result
        == 'https://github.com/test-owner/test-repo/compare/ide-fix-issue-42-try3?expand=1'
    )


@patch('ide.resolver.send_pull_request.argparse.ArgumentParser')
@patch('ide.resolver.send_pull_request.process_single_issue')
@patch('ide.resolver.send_pull_request.load_single_resolver_output')
@patch('ide.resolver.send_pull_request.identify_token')
