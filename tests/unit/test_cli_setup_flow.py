...
from ide.cli.main import run_setup_flow
...
class TestCLISetupFlow(unittest.TestCase):
    ...
    @patch('ide.cli.settings.modify_llm_settings_basic')
    @patch('ide.cli.main.print_formatted_text')
    ...
    @patch('ide.cli.main.print_formatted_text')
    @patch('ide.cli.main.run_setup_flow')
    ...
    async def test_main_calls_setup_flow_when_no_settings(
        ...
        # Setup
        mock_args = MagicMock()
        mock_config = MagicMock(spec=IDEConfig)
