...
from ide.integrations.service_types import GitService
from ide.server.routes.mcp import get_convo_link
...
async def test_get_convo_link_non_saas_mode():
    ...
    with patch('ide.server.routes.mcp.server_config') as mock_config:
...
async def test_get_convo_link_saas_mode():
    ...
    # Test with SAAS mode
    with (
        patch('ide.server.routes.mcp.server_config') as mock_config,
        patch(
            'ide.server.routes.mcp.CONVO_URL',
