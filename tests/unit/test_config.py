...
from ide.core.config.condenser_config import (
    LLMSummarizingCondenserConfig,
    NoOpCondenserConfig,
    RecentEventsCondenserConfig,
)
from ide.core.logger import ide_logger
...
def test_compat_env_to_config(monkeypatch, setup_env):
    ...
    assert config.sandbox.volumes == '/repos/ide/workspace:/workspace:rw'
    # Check that the old parameters are set for backward compatibility
    assert config.workspace_base == os.path.abspath('/repos/ide/workspace')
    assert config.workspace_mount_path == os.path.abspath('/repos/ide/workspace')
...
[llm]
# Not set in `ide/core/schema/config.py`
invalid_field = "test"
model = "gpt-4"
...
""")
    ...
    try:
        ...
        ide_logger.removeHandler(handler)
...
def test_agent_config_condenser_with_no_enabled():
    ...
    config = IDEConfig(enable_default_condenser=False)
...
def test_api_keys_repr_str():
    ...
    # Check that no other attrs in IDEConfig have 'key' or 'token' in their name
    # This will fail when new attrs are added, and attract attention
    known_key_token_attrs_app = [
        'search_api_key',
    ]
    for attr_name in IDEConfig.model_fields.keys():
        if (
            not attr_name.startswith('__')
            and attr_name not in known_key_token_attrs_app
        ):
            assert 'key' not in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'key' in IDEConfig"
            )
            assert 'token' not in attr_name.lower() or 'tokens' in attr_name.lower(), (
                f"Unexpected attribute '{attr_name}' contains 'token' in IDEConfig"
            )
...
"""
    ...
    app_config = load_ide_config(config_file=temp_toml_file)
