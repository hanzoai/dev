...
from ide.integrations.service_types import (
    AuthenticationError,
    ProviderType,
    SuggestedTask,
    TaskType,
...
from ide.server.data_models.conversation_info import ConversationInfo
from ide.server.data_models.conversation_info_result_set import (
    ConversationInfoResultSet,
)
from ide.server.routes.manage_conversations import (
...
from ide.server.routes.manage_conversations import app as conversation_app
from ide.server.types import LLMAuthenticationError, MissingSettingsError
from ide.server.user_auth.user_auth import AuthType
from ide.storage.data_models.conversation_metadata import (
    ConversationMetadata,
    ConversationTrigger,
)
from ide.storage.data_models.conversation_status import ConversationStatus
from ide.storage.locations import get_conversation_metadata_filename
from ide.storage.memory import InMemoryFileStore
...
def _patch_store():
    ...
    with patch(
        'ide.storage.conversation.file_conversation_store.get_file_store',
        MagicMock(return_value=file_store),
    ):
        with patch(
            'ide.server.routes.manage_conversations.conversation_manager.file_store',
            file_store,
        ):
            yield
...
def provider_handler_mock():
    with patch('ide.server.routes.manage_conversations.ProviderHandler') as mock_cls:
...
async def test_search_conversations():
    with _patch_store():
        with patch('ide.server.routes.manage_conversations.config') as mock_config:
            mock_config.conversation_max_age_seconds = 864000  # 10 days
            with patch(
                'ide.server.routes.manage_conversations.conversation_manager'
            ) as mock_manager:
                ...
                with patch(
                    'ide.server.routes.manage_conversations.datetime'
                ) as mock_datetime:
...
async def test_new_conversation_success(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            'ide.server.routes.manage_conversations.create_new_conversation'
...
async def test_new_conversation_with_suggested_task(provider_handler_mock):
    ...
    with _patch_store():
        ...
        with patch(
            ...
            with patch(
                'ide.integrations.service_types.SuggestedTask.get_prompt_for_task'
            ) as mock_get_prompt:
...
async def test_delete_conversation():
    with _patch_store():
        ...
        with patch(
            'ide.server.routes.manage_conversations.ConversationStoreImpl.get_instance'
            ...
            with patch(
                ...
                with patch(
                    'ide.server.routes.manage_conversations.get_runtime_cls'
