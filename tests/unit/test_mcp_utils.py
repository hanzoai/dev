...
import ide.mcp.utils
from ide.core.config.mcp_config import MCPSSEServerConfig
...
async def test_create_mcp_clients_empty():
    ...
    clients = await ide.mcp.utils.create_mcp_clients([], [])
    assert clients == []


@pytest.mark.asyncio
@patch('ide.mcp.utils.MCPClient')
async def test_create_mcp_clients_success(mock_mcp_client):
    ...
    clients = await ide.mcp.utils.create_mcp_clients(server_configs, [])
...
def test_convert_mcp_clients_to_tools_empty():
    ...
    tools = ide.mcp.utils.convert_mcp_clients_to_tools(None)
    assert tools == []

    tools = ide.mcp.utils.convert_mcp_clients_to_tools([])
    assert tools == []
...
def test_convert_mcp_clients_to_tools():
    ...
    # Convert to tools
    tools = ide.mcp.utils.convert_mcp_clients_to_tools([mock_client1, mock_client2])
...
async def test_call_tool_mcp_no_clients():
    ...
    with pytest.raises(ValueError, match='No MCP clients found'):
        await ide.mcp.utils.call_tool_mcp([], action)
...
async def test_call_tool_mcp_no_matching_client():
    ...
    with pytest.raises(ValueError, match='No matching MCP agent found for tool name'):
        await ide.mcp.utils.call_tool_mcp([mock_client], action)
...
async def test_call_tool_mcp_success():
    ...
    # Call the function
    observation = await ide.mcp.utils.call_tool_mcp([mock_client], action)
