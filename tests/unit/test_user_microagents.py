...
from ide.microagent import KnowledgeMicroagent, MicroagentType, RepoMicroagent
from ide.storage import get_file_store


@pytest.fixture
def temp_user_microagents_dir():
    """Create a temporary directory to simulate ~/.ide/microagents/."""
...
def test_user_microagents_loading(temp_user_microagents_dir):
    """Test that user microagents are loaded from ~/.ide/microagents/."""
    with patch(
        'ide.memory.memory.USER_MICROAGENTS_DIR', str(temp_user_microagents_dir)
    ):
        with tempfile.TemporaryDirectory() as temp_dir:
...
def test_user_microagents_directory_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('ide.memory.memory.USER_MICROAGENTS_DIR', str(non_existent_dir)):
...
"""
        ...
        with patch('ide.memory.memory.USER_MICROAGENTS_DIR', str(user_dir)):
            with tempfile.TemporaryDirectory() as temp_store_dir:
                # Create event stream and memory
                file_store = get_file_store('local', temp_store_dir)
                event_stream = EventStream('test', file_store)
...
def test_user_microagents_empty_directory():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('ide.memory.memory.USER_MICROAGENTS_DIR', str(empty_dir)):
            with tempfile.TemporaryDirectory() as temp_store_dir:
                # Create event stream and memory
                file_store = get_file_store('local', temp_store_dir)
                event_stream = EventStream('test', file_store)
