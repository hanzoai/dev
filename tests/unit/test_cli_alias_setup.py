...
from ide.cli.shell_config import (
    ShellConfigManager,
...
def test_get_shell_config_path_no_files_fallback():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch('ide.cli.shell_config.Path.home', return_value=Path(temp_dir)):
        ...
        with patch('ide.cli.shell_config.Path.home', return_value=Path(temp_dir)):
            ...
            with patch('shellingham.detect_shell', return_value=('bash', 'bash')):
                ...
                with open(profile_path, 'r') as f:
                    content = f.read()
                    assert 'alias ide=' in content
                    assert 'alias oh=' in content
                    assert 'uvx --python 3.12 --from hanzo-dev hanzo-dev' in content
        ...
        with patch('ide.cli.shell_config.Path.home', return_value=Path(temp_dir)):
            ...
            with patch('shellingham.detect_shell', return_value=('bash', 'bash')):
                ...
                with open(profile_path, 'r') as f:
                    ...
                    ide_count = content.count('alias ide=')
        ...
        with patch('ide.cli.shell_config.Path.home', return_value=Path(temp_dir)):
            ...
            with patch.object(manager, 'detect_shell', return_value='bash'):
                ...
                with open(bashrc, 'r') as f:
                    ...
                    assert 'alias ide="test-command"' in content
