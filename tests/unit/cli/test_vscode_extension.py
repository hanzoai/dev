...
from ide.cli import vscode_extension
...
def mock_env_and_dependencies():
    ...
    with (
        ...
        mock.patch(
            'ide.cli.vscode_extension.download_latest_vsix_from_github'
        ) as mock_download,
        mock.patch('builtins.print') as mock_print,
        mock.patch('ide.cli.vscode_extension.logger.debug') as mock_logger,
    ):
        ...
        try:
            ...
            ide_dir = temp_dir / '.ide'
            if ide_dir.exists():
                for f in ide_dir.glob('*'):
                    if f.is_file():
                        f.unlink()
                try:
                    ide_dir.rmdir()
...
def test_extension_already_installed_detected(mock_env_and_dependencies):
    ...
    mock_env_and_dependencies['print'].assert_any_call(
        'INFO: IDE VS Code extension is already installed.'
    )
...
def test_installation_failure_no_flag_created(mock_env_and_dependencies):
    ...
    mock_env_and_dependencies['print'].assert_any_call(
        'INFO: Will retry installation next time you run IDE in VS Code.'
    )
...
def test_bundled_fails_falls_back_to_github(mock_env_and_dependencies):
    ...
    with (
        ...
        mock_env_and_dependencies['print'].assert_any_call(
            'INFO: IDE VS Code extension installed successfully from GitHub.'
        )
...
def test_all_methods_fail(mock_env_and_dependencies):
    ...
    mock_env_and_dependencies['print'].assert_any_call(
        'INFO: Automatic installation failed. Please check the IDE documentation for manual installation instructions.'
    )
    mock_env_and_dependencies['print'].assert_any_call(
        'INFO: Will retry installation next time you run IDE in VS Code.'
    )
...
def test_windsurf_detection_and_install(mock_env_and_dependencies):
    ...
    mock_env_and_dependencies['print'].assert_any_call(
        'INFO: Will retry installation next time you run IDE in Windsurf.'
    )
...
def test_bundled_vsix_installation_failure_fallback_to_marketplace(
    ...
    mock_vsix_path = mock.MagicMock()
    mock_vsix_path.exists.return_value = True
    mock_vsix_path.__str__.return_value = '/mock/path/ide-vscode-0.0.1.vsix'
    mock_env_and_dependencies[
        'as_file'
    ].return_value.__enter__.return_value = mock_vsix_path
    ...
    mock_env_and_dependencies['subprocess'].side_effect = [
        ...
        subprocess.CompletedProcess(
            args=[
                'code',
                '--install-extension',
                '/mock/path/ide-vscode-0.0.1.vsix',
                '--force',
            ],
            returncode=1,
