...
from ide.core.config.utils import load_from_toml
...
def test_app_config_extended_from_toml(tmp_path: os.PathLike) -> None:
    ...
    This test verifies that the [extended] section is loaded into IDEConfig.extended
...
"""
    ...
    # Load the TOML into the IDEConfig instance
    config = IDEConfig()
...
def test_app_config_extended_default(tmp_path: os.PathLike) -> None:
    ...
    IDEConfig.extended remains its default (empty) ExtendedConfig.
    """
    config_content = """
[core]
workspace_base = "/tmp/workspace"
