...
def _check_source_code_in_dir(temp_dir):
    ...
    # The source code should only include the `ide` folder,
    ...
    assert os.path.exists(os.path.join(code_dir, 'ide'))
    assert os.path.isdir(os.path.join(code_dir, 'ide'))
    ...
    assert _pyproject_version == version('ide-ai')
...
def test_generate_dockerfile_build_from_scratch():
    ...
    assert 'COPY ./code/ide /ide/code/ide' in dockerfile_content
    assert (
        '/ide/micromamba/bin/micromamba run -n ide poetry install' in dockerfile_content
    )
...
def live_docker_image():
    ...
    RUN echo "Hello, IDE!" > /ide/hello.txt
    """
