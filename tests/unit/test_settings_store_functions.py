...
from ide.integrations.provider import ProviderToken
from ide.integrations.service_types import ProviderType
from ide.server.routes.secrets import (
    app,
    check_provider_tokens,
)
from ide.server.routes.settings import store_llm_settings
...
from ide.storage.data_models.user_secrets import UserSecrets
from ide.storage.secrets.file_secrets_store import FileSecretsStore
...
def test_client():
    ...
    with (
        ...
        patch('ide.server.dependencies._SESSION_API_KEY', None),
        patch(
            'ide.server.routes.secrets.check_provider_tokens',
...
def file_secrets_store(temp_dir):
    ...
    with patch(
        'ide.storage.secrets.file_secrets_store.FileSecretsStore.get_instance',
...
async def test_check_provider_tokens_valid():
    ...
    with patch('ide.server.routes.secrets.validate_provider_token') as mock_validate:
