...
from ide.core.config import IDEConfig, LLMConfig
from ide.core.config.agent_config import AgentConfig
from ide.events import EventStream, EventStreamSubscriber
...
async def test_agent_session_start_with_no_state(mock_agent):
    ...
    with (
        ...
        patch(
            'ide.server.session.agent_session.EventStream',
            return_value=mock_event_stream,
        ),
        patch(
            'ide.controller.state.state.State.restore_from_session',
            side_effect=Exception('No state found'),
        ),
        patch('ide.server.session.agent_session.Memory', return_value=memory),
    ):
        await session.start(
...
async def test_agent_session_start_with_restored_state(mock_agent):
    ...
    with (
        ...
        patch('ide.server.session.agent_session.Memory', mock_memory),
    ):
        await session.start(
            runtime_name='test-runtime',
            config=IDEConfig(),
