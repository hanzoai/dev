...
from hanzo.core.logger import HanzoLoggerAdapter, json_log_handler
from hanzo.core.logger import hanzo_logger as hanzo_logger
...
def json_handler():
    ...
    hanzo_logger.addHandler(json_handler)
    yield hanzo_logger, stream
    hanzo_logger.removeHandler(json_handler)
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict('hanzo.core.logger.os.environ', environ, clear=True):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = HanzoLoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = HanzoLoggerAdapter(logger, extra={'override': 'a'})
