...
from ide.core.logger import (
    LOG_JSON_LEVEL_KEY,
    IDELoggerAdapter,
    json_log_handler,
)
from ide.core.logger import ide_logger as ide_logger
...
def json_handler():
    ...
    ide_logger.addHandler(json_handler)
    yield ide_logger, stream
    ide_logger.removeHandler(json_handler)
...
def test_app_config_attributes_masking(test_handler):
    logger, stream = test_handler
    app_config = IDEConfig(search_api_key='search-xyz789')
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict('ide.core.logger.os.environ', environ, clear=True):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = IDELoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = IDELoggerAdapter(logger, extra={'override': 'a'})
