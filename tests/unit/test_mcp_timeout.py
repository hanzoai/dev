...
from ide.core.config.mcp_config import MCPConfig, MCPSSEServerConfig
from ide.mcp import create_mcp_clients, fetch_mcp_tools_from_config
from hanzo_mcp_client import MCPClient
...
async def test_sse_connection_timeout():
    ...
    with mock.patch('ide.mcp.utils.MCPClient', return_value=mock_client):
...
async def test_fetch_mcp_tools_with_timeout():
    ...
    with mock.patch('ide.mcp.utils.create_mcp_clients', return_value=[]):
...
async def test_mixed_connection_results():
    ...
    with mock.patch(
        'ide.mcp.utils.create_mcp_clients', return_value=[successful_client]
    ):
        # Call fetch_mcp_tools_from_config
        tools = await fetch_mcp_tools_from_config(mock_config, None)
