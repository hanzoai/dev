...
from ide.events.action import CmdRunAction, MessageAction, RecallAction
from ide.events.action.agent import CondensationAction
...
from ide.memory.condenser.condenser import Condensation, View
...
def create_events(event_data):
    ...
    from ide.events.action import CmdRunAction, RecallAction
    from ide.events.observation import CmdOutputObservation, RecallObservation
...
def test_only_dangling_observations_in_recent_slice(condenser_fixture):
    ...
    # Forgotten IDs: [5, 6]
    with patch(
        'ide.memory.condenser.impl.conversation_window_condenser.logger.warning'
    ) as mock_log_warning:
        condensation = condenser.get_condensation(view)
