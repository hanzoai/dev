...
from ide.events.observation import NullObservation, Observation
from ide.events.stream import EventStream
from ide.integrations.provider import ProviderHandler, ProviderToken, ProviderType
from ide.integrations.service_types import AuthenticationError, Repository
...
class TestRuntime(Runtime):
    ...
    run_action_calls = []
    _execute_shell_fn_git_handler = MagicMock(
...
async def test_clone_or_init_repo_github_with_token(temp_dir, monkeypatch):
    ...
    assert 'git checkout -b ide-workspace-' in checkout_cmd
