...
from ide.core.config import CLIConfig, IDEConfig


class TestCliViMode:
    """Test the VI mode feature."""

    @patch('ide.cli.tui.PromptSession')
    def test_create_prompt_session_vi_mode_enabled(self, mock_prompt_session):
        """Test that vi_mode can be enabled."""
        from ide.cli.tui import create_prompt_session

        config = IDEConfig(cli=CLIConfig(vi_mode=True))
    ...
    def test_create_prompt_session_vi_mode_disabled(self, mock_prompt_session):
        ...
        config = IDEConfig(cli=CLIConfig(vi_mode=False))
        create_prompt_session(config)
        mock_prompt_session.assert_called_with(
            style=ANY,
            vi_mode=False,
    ...
    def test_cli_confirm_vi_keybindings_are_added(self, mock_app_class):
        ...
        from ide.cli.tui import cli_confirm

        config = IDEConfig(cli=CLIConfig(vi_mode=True))
        with patch('ide.cli.tui.KeyBindings', MagicMock()) as mock_key_bindings:
            cli_confirm(
    ...
    def test_vi_mode_disabled_by_default(self):
        ...
        from ide.core.config.utils import load_from_env
