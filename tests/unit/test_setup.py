...
from ide.events.action import CmdRunAction, FileReadAction
from ide.events.event import EventSource
from ide.events.observation import ErrorObservation, FileReadObservation
...
def test_maybe_run_setup_script_executes_action():
    ...
    runtime.read.assert_called_once_with(FileReadAction(path='.ide/setup.sh'))
    ...
    action = args[0]
    assert isinstance(action, CmdRunAction)
    assert action.command == 'chmod +x .ide/setup.sh && source .ide/setup.sh'
