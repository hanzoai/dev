...
from ide.cli.tui import process_agent_pause
from ide.core.schema import AgentState
from ide.events import EventSource
from ide.events.action import ChangeAgentStateAction
from ide.events.observation import AgentStateChangedObservation


class TestProcessAgentPause:
    @pytest.mark.asyncio
    @patch('ide.cli.tui.create_input')
...
class TestCliPauseResumeInRunSession:
    ...
    async def test_on_event_async_pause_processing(self):
        ...
        with (
            patch('ide.cli.main.display_event') as mock_display_event,
            patch('ide.cli.main.update_usage_metrics') as mock_update_metrics,
        ):
...
class TestCliCommandsPauseResume:
    @pytest.mark.asyncio
    @patch('ide.cli.commands.handle_resume_command')
    async def test_handle_commands_resume(self, mock_handle_resume):
        ...
        from ide.cli.commands import handle_commands
...
class TestAgentStatePauseResume:
    @pytest.mark.asyncio
    @patch('ide.cli.main.display_agent_running_message')
    @patch('ide.cli.tui.process_agent_pause')
    ...
    @pytest.mark.asyncio
    @patch('ide.cli.main.display_event')
    @patch('ide.cli.main.update_usage_metrics')
