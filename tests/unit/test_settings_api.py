...
from dev.integrations.provider import ProviderType, SecretStore
from dev.server.app import app
from dev.server.settings import Settings


@pytest.fixture
def mock_settings_store():
    with patch('dev.server.routes.settings.SettingsStoreImpl') as mock:
...
def mock_get_user_id():
    with patch('dev.server.routes.settings.get_user_id') as mock:
...
def mock_validate_provider_token():
    with patch('dev.server.routes.settings.validate_provider_token') as mock:
...
def mock_github_service():
    with patch('dev.server.routes.settings.GitHubService') as mock:
        yield mock
...
async def test_settings_api_runtime_factor(
    ...
    with patch('dev.server.shared.config') as mock_config:
