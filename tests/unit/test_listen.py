...
def test_load_file_upload_config_invalid_max_size():
    ...
    with patch('hanzo.server.shared.config', config):
...
def test_is_extension_allowed():
    with (
        patch('hanzo.server.file_config.RESTRICT_FILE_TYPES', True),
        patch('hanzo.server.file_config.ALLOWED_EXTENSIONS', ['.txt', '.pdf']),
    ):
...
def test_is_extension_allowed_no_restrictions():
    with patch('hanzo.server.file_config.RESTRICT_FILE_TYPES', False):
        assert is_extension_allowed('file.txt')
        assert is_extension_allowed('file.pdf')
...
def test_is_extension_allowed_wildcard():
    with (
        patch('hanzo.server.file_config.RESTRICT_FILE_TYPES', True),
        patch('hanzo.server.file_config.ALLOWED_EXTENSIONS', ['.*']),
    ):
        assert is_extension_allowed('file.txt')
        assert is_extension_allowed('file.pdf')
        assert is_extension_allowed('file.doc')
