...
from ide.controller.agent_controller import AgentController
from ide.controller.state.control_flags import (
    BudgetControlFlag,
)
...
from ide.core.main import run_controller
from ide.core.schema import AgentState
from ide.events import Event, EventSource, EventStream, EventStreamSubscriber
from ide.events.action import ChangeAgentStateAction, CmdRunAction, MessageAction
from ide.events.action.agent import CondensationAction, RecallAction
from ide.events.action.message import SystemMessageAction
from ide.events.event import RecallType
from ide.events.observation import (
    AgentStateChangedObservation,
    ErrorObservation,
)
from ide.events.observation.agent import RecallObservation
from ide.events.observation.empty import NullObservation
from ide.events.serialization import event_to_dict
from ide.llm import LLM
from ide.llm.metrics import Metrics, TokenUsage
from ide.memory.condenser.condenser import Condensation
from ide.memory.condenser.impl.conversation_window_condenser import (
    ConversationWindowCondenser,
)
from ide.memory.memory import Memory
from ide.memory.view import View
...
def mock_agent():
    ...
    agent.llm.config = IDEConfig().get_llm_config()
...
async def test_context_window_exceeded_error_handling(
    ...
    config = IDEConfig(max_iterations=max_iterations)
...
async def test_run_controller_with_context_window_exceeded_with_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = IDEConfig(max_iterations=5)
    mock_runtime.config = copy.deepcopy(config)
...
async def test_run_controller_with_context_window_exceeded_without_truncation(
    ...
    mock_runtime.event_stream = test_event_stream
    config = IDEConfig(max_iterations=3)
    mock_runtime.config = copy.deepcopy(config)
    try:
        state = await asyncio.wait_for(
            run_controller(
...
async def test_first_user_message_with_identical_content(test_event_stream, mock_agent):
    ...
    mock_agent.llm = MagicMock(spec=LLM)
    mock_agent.llm.metrics = Metrics()
    mock_agent.llm.config = IDEConfig().get_llm_config()
