...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        'ide.storage.data_models.settings.load_ide_config',
        MagicMock(return_value=IDEConfig()),
    ):
...
async def test_get_instance():
    config = IDEConfig(file_store='local', file_store_path='/test/path')

    with patch(
        'ide.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
