...
def prompt_dir(tmp_path):
    # Copy contents from "ide/agenthub/codeact_agent" to the temp directory
    shutil.copytree('ide/agenthub/codeact_agent/prompts', tmp_path, dirs_exist_ok=True)
...
"""
    ...
    with patch('ide.memory.memory.GLOBAL_MICROAGENTS_DIR', test_microagents_dir):
        # Initialize Memory
        memory = Memory(
            event_stream=event_stream,
            sid='test-session',
...
async def test_conversation_instructions_plumbed_to_memory(
    ...
    with (
        patch('ide.server.session.agent_session.AgentController', SpyAgentController),
    ):
        await session.start(
