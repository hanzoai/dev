...
def test_simple_cmd_ipython_and_fileop(temp_dir, runtime_cls, run_as_hanzo):
    ...
    assert obs.content.strip() == (
        ...
        '[Jupyter Python interpreter: /hanzo/poetry/hanzo-ai-5O4_aCHf-py3.12/bin/python]'
    )
...
def test_ipython_multi_user(temp_dir, runtime_cls, run_as_hanzo):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_hanzo)
    ...
    assert obs.exit_code == 0
    if run_as_hanzo:
        # -rw-r--r-- 1 hanzo root 13 Jul 28 03:53 test.txt
        assert 'hanzo' in obs.content.split('\r\n')[0]
    else:
...
def test_ipython_package_install(temp_dir, runtime_cls, run_as_hanzo):
...
def test_ipython_file_editor_permissions_as_hanzo(temp_dir, runtime_cls):
    """Test file editor permission behavior when running as different users."""
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_hanzo=True)
    ...
    # Try to view the file as hanzo user - should fail with permission denied
    ...
    # Try to edit the file as hanzo user - should fail with permission denied
    ...
    # Try to use file editor in hanzo sandbox directory - should work
    test_code = """
# Create file
