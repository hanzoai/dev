...
from ide.core.config import IDEConfig, MCPConfig, load_ide_config
from ide.core.logger import ide_logger as logger
from ide.events import EventStream
...
from ide.runtime.plugins import AgentSkillsRequirement, JupyterRequirement
...
RUN_AS_IDE = os.getenv('RUN_AS_IDE', 'True').lower() in ['true', '1', 'yes']
test_mount_path = ''
...
def get_run_as_ide() -> list[bool]:
    print(
        '\n\n########################################################################'
    )
    print('USER: ' + 'ide' if RUN_AS_IDE else 'root')
    print(
        '########################################################################\n\n'
    )
    return [RUN_AS_IDE]
...
@pytest.fixture(scope='module', params=get_run_as_ide())
def run_as_ide(request):
    time.sleep(1)
    return request.param
