...
from ide.core.config import MCPConfig
from ide.core.config.mcp_config import MCPStdioServerConfig
from ide.mcp.utils import add_mcp_tools_to_agent
...
from ide.microagent.types import MicroagentType


def _create_test_microagents(test_dir: str):
    """Create test microagent files in the given directory."""
    microagents_dir = Path(test_dir) / '.ide' / 'microagents'
...
"""
    (Path(test_dir) / '.ide_instructions').write_text(legacy_instructions)


def test_load_microagents_with_trailing_slashes(temp_dir, runtime_cls, run_as_ide):
    ...
    try:
        ...
        assert len(repo_agents) == 2  # repo.md + .ide_instructions
...
def test_load_microagents_with_selected_repo(temp_dir, runtime_cls, run_as_ide):
    """Test loading microagents from a selected repository."""
    # Create test files in a repository-like structure
    repo_dir = Path(temp_dir) / 'IDE'
    ...
    try:
        ...
        loaded_agents = runtime.get_microagents_from_selected_repo('hanzoai/ide')
...
def test_load_microagents_with_missing_files(temp_dir, runtime_cls, run_as_ide):
    """Test loading microagents when some files are missing."""
    # Create only repo.md, no other files
    microagents_dir = Path(temp_dir) / '.ide' / 'microagents'
    microagents_dir.mkdir(parents=True, exist_ok=True)
...
def test_default_tools_microagent_exists():
    ...
    project_root = os.path.dirname(ide.__file__)
...
async def test_add_mcp_tools_from_microagents():
    ...
    with patch(
        'ide.mcp.utils.fetch_mcp_tools_from_config',
        new=AsyncMock(return_value=[mock_tool]),
    ):
        # Call the function with the IDEConfig instead of MCPConfig
