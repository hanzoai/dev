...
from hanzo.events.action import FileEditAction, FileReadAction
from hanzo.events.observation import FileEditObservation
...
def test_edit_from_scratch(temp_dir, runtime_cls, run_as_hanzo):
...
def test_edit(temp_dir, runtime_cls, run_as_hanzo):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_hanzo)
    try:
        action = FileEditAction(
            content=ORGINAL,
...
def test_edit_long_file(temp_dir, runtime_cls, run_as_hanzo):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_hanzo)
    try:
        action = FileEditAction(
