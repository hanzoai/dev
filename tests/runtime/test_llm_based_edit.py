...
from ide.events.action import FileEditAction, FileReadAction
...
def test_edit_from_scratch(temp_dir, runtime_cls, run_as_ide):
...
def test_edit(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        action = FileEditAction(
            content=ORGINAL,
...
def test_edit_long_file(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        action = FileEditAction(
