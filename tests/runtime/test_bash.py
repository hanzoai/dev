...
from ide.events.action import CmdRunAction
from ide.events.observation import CmdOutputObservation, ErrorObservation
from ide.runtime.impl.cli.cli_runtime import CLIRuntime
from ide.runtime.impl.local.local_runtime import LocalRuntime
from ide.runtime.utils.bash_constants import TIMEOUT_MESSAGE_TEMPLATE
...
def test_bash_server(temp_dir, runtime_cls, run_as_ide):
...
def test_bash_background_server(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    server_port = 8081
    try:
...
def test_complex_commands(temp_dir, runtime_cls, run_as_ide):
...
def test_no_ps2_in_output(temp_dir, runtime_cls, run_as_ide):
...
def test_multiline_command_loop(temp_dir, runtime_cls):
    # https://github.com/hanzoai/ide/issues/3143
...
def test_multiple_multiline_commands(temp_dir, runtime_cls, run_as_ide):
    if is_windows():
        cmds = [
            'Get-ChildItem',
...
def test_cliruntime_multiple_newline_commands(temp_dir, run_as_ide):
...
def test_cmd_run(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        if is_windows():
            ...
            obs = _run_cmd_action(runtime, 'ls -l')
            assert obs.exit_code == 0
            if run_as_ide and runtime_cls != CLIRuntime and runtime_cls != LocalRuntime:
                assert 'ide' in obs.content
...
def test_run_as_user_correct_home_dir(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        if is_windows():
            # Windows PowerShell version
            ...
            if runtime_cls == LocalRuntime:
                ...
                assert '/home/ide' in obs.content
            else:
                assert '/root' in obs.content
    finally:
        _close_test_runtime(runtime)
...
def test_git_operation(temp_dir, runtime_cls):
    ...
    try:
        if runtime_cls != LocalRuntime and runtime_cls != CLIRuntime:
            ...
            obs = _run_cmd_action(runtime, 'sudo chown -R ide:root .')
            assert obs.exit_code == 0

        # check the ownership of the current directory
        obs = _run_cmd_action(runtime, 'ls -alh .')
        assert obs.exit_code == 0
        # drwx--S--- 2 ide root   64 Aug  7 23:32 .
        ...
        for line in obs.content.split('\n'):
            ...
            if ' ..' in line:
                ...
                assert 'root' in line
                assert 'ide' not in line
            elif ' .' in line:
                # current directory should be owned by ide
        ...
        if runtime_cls == LocalRuntime or runtime_cls == CLIRuntime:
            ...
            obs = _run_cmd_action(
                runtime,
                'git config user.name "ide" && git config user.email "ide@hanzo.ai"',
            )
            assert obs.exit_code == 0
...
def test_python_version(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_pwd_property(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_basic_command(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        if is_windows():
            # Test simple command
...
def test_interactive_command(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(
        temp_dir,
        runtime_cls,
        run_as_ide,
...
def test_long_output(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_long_output_exceed_history_limit(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        # Generate a long output
...
def test_long_output_from_nested_directories(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_command_backslash(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_stress_long_output_with_soft_and_hard_timeout(
    temp_dir, runtime_cls, run_as_ide
):
    runtime, config = _load_runtime(
        temp_dir,
        runtime_cls,
    ...
    try:
        ...
        for i in range(10):
            ...
            mem_action = CmdRunAction(
                'ps aux | awk \'{printf "%8.1f KB  %s\\n", $6, $0}\' | sort -nr | grep "action_execution_server" | grep "/ide/poetry" | grep -v grep | awk \'{print $1}\''
            )
            mem_obs = runtime.run_action(mem_action)
            assert mem_obs.exit_code == 0
            logger.info(
...
def test_command_output_continuation(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        if is_windows():
            # Windows PowerShell version
...
def test_long_running_command_follow_by_execute(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        if is_windows():
...
def test_empty_command_errors(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_python_interactive_input(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_python_interactive_input_without_set_input(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
...
def test_bash_remove_prefix(temp_dir, runtime_cls, run_as_ide):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_ide)
    try:
        # create a git repo - same for both platforms
        action = CmdRunAction(
            'git init && git remote add origin https://github.com/hanzoai/ide'
        )
        obs = runtime.run_action(action)
        # logger.info(obs, extra={'msg_type': 'OBSERVATION'})
        assert obs.metadata.exit_code == 0
        ...
        # logger.info(obs, extra={'msg_type': 'OBSERVATION'})
        assert obs.metadata.exit_code == 0
        assert 'https://github.com/hanzoai/ide' in obs.content
