...
/// Return a long version string suitable for `--version` output.
pub fn long_version() -> &'static str {
    static LONG: LazyLock<String> = LazyLock::new(|| {
        let mut parts = vec![CODE_VERSION.to_string()];
...
/// Return the git describe tag if available at build time.
pub fn git_describe() -> Option<&'static str> {
    option_env!("CODE_GIT_DESCRIBE")
}

/// Return the git SHA if available at build time.
pub fn git_sha() -> Option<&'static str> {
    option_env!("CODE_GIT_SHA")
}

/// Return the git commit date if available at build time.
pub fn git_date() -> Option<&'static str> {
    option_env!("CODE_GIT_DATE")
}
