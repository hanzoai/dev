    ...
    pub async fn get_or_create_page(&self) -> Result<Arc<Page>> {
        ...
        *page_guard = Some(Arc::clone(&page));
        ...
        let console_hook = r#"(function(){
            try {
                if (!window.__dev_console_logs) {
                    window.__dev_console_logs = [];
                    const push = (level, message) => {
                        try {
                            window.__dev_console_logs.push({ timestamp: new Date().toISOString(), level, message });
                            if (window.__dev_console_logs.length > 2000) window.__dev_console_logs.shift();
                        } catch (_) {}
                    };
        ...
        let script = format!(
            r#"(function() {{
                try {{
                    ...
                    var logs = window.__dev_console_logs || [];
                    var n = {requested};
    ...
    pub async fn cleanup(&self) -> Result<()> {
        ...
        let cleanup_js = r#"
            (function(){
                ...
                try { if (window.__dev_console_logs) delete window.__dev_console_logs; } catch(_) {}
                return true;
            })()
        "#;
            ...
            loop {
                ...
                if cfg_now.connect_port.is_some() || cfg_now.connect_ws.is_some() {
                    ...
                    let listener_script = r#"
                        (function(){
                          try {
                            if (!window.__dev_nav_listening) {
                              window.__dev_nav_listening = true;
                              window.__dev_nav_seq = 0;
                              window.__dev_nav_url = String(location.href || '');
                              window.addEventListener('codex:locationchange', function(){
                                window.__dev_nav_seq += 1;
                                window.__dev_nav_url = String(location.href || '');
                              }, { capture: true });
                            }
                            return { seq: Number(window.__dev_nav_seq||0), url: String(window.__dev_nav_url||location.href) };
