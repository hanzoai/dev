...
pub enum Command {
    /// Submit a new Hanzo Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Hanzo Cloud task.
    Status(StatusCommand),
    /// List Hanzo Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Hanzo Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Hanzo Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Hanzo Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `hanzo cloud` to browse).
    ...
    /// Git branch to run in Hanzo Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Hanzo Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Hanzo Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Hanzo Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
