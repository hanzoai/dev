...
use hanzo_utils_image::load_and_resize_to_fit;
...
use hanzo_utils_image::error::ImageProcessingError;
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Cannot attach image at `{}`: unsupported image format `{}`.",
            path.display(),
            mime
        ),
    }
...
mod tests {
    ...
    fn empty_exec_policy() -> Policy {
        hanzo_execpolicy::get_default_policy().expect("default policy")
    }
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Cannot attach image at `{}`: unsupported image format `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
