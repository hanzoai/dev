...
fn build_compact_tokens_line(used: u64, limit: u64) -> Line<'static> {
    ...
    let used_fmt = format_with_separators(used as i64);
    let limit_fmt = format_with_separators(limit as i64);
    spans.push(Span::styled(
...
fn build_compact_status_line(used: u64, limit: u64) -> Line<'static> {
        ...
        return Line::from(vec![
            ...
            Span::styled(
                format!(
                    ...
                    format_with_separators(remaining as i64)
                ),
                Style::default().fg(colors::dim()),
            ),
        ]);
    ...
    Line::from(vec![
        Span::raw(field_prefix("Status")),
        Span::styled(
            format!("Exceeded by {} tokens", format_with_separators(overage as i64)),
...
fn build_context_tokens_line(used: u64, window: u64) -> Line<'static> {
    ...
    spans.push(Span::styled(
        format!(
            " ({} / {})",
            format_with_separators(used as i64),
            format_with_separators(window as i64)
        ),
        Style::default().fg(colors::dim()),
    ));
    Line::from(spans)
...
fn build_context_status_line(used: u64, window: u64, overflow_auto_compact: bool) -> Line<'static> {
    ...
    let mut message = format!("Exceeded by {} tokens", format_with_separators(overage as i64));
