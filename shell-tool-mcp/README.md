...
**Note: This MCP server is still experimental. When using it with Hanzo Dev CLI, ensure the CLI version matches the MCP server version.**
...
The user can use [Hanzo Dev `.rules`](https://developers.openai.com/codex/local-config#rules-preview) files to define how a command should be handled. The action to take is determined by the `decision` parameter of a matching rule as follows:
...
Because `@openai/codex-shell-tool-mcp` intercepts `execve(2)` calls directly, it _always_ knows the full path to the program being executed. In turn, this makes it possible to provide stronger guarantees on how [Hanzo Dev `.rules`](https://developers.openai.com/codex/local-config#rules-preview) are enforced.
...
To test out the MCP with a one-off invocation of Hanzo Dev CLI, it is important to _disable_ the default shell tool in addition to enabling the MCP so Hanzo Dev has exactly one shell-like tool available to it:

```bash
dev --disable shell_tool \
  --config 'mcp_servers.bash={command = "npx", args = ["-y", "@openai/codex-shell-tool-mcp"]}'
```

To configure this permanently so you can use the MCP while running `dev` without additional command-line flags, add the following to your `~/.hanzo/config.toml` (legacy `~/.code/config.toml` or `~/.codex/config.toml` are still read):
...
This MCP server is designed to be used with [Hanzo Dev](https://developers.openai.com/codex/cli), as it declares the following `capability` that Hanzo Dev supports when acting as an MCP client:
...
{
  "id": "req-42",
  "method": "dev/sandbox-state/update",
  "params": {
    "sandboxPolicy": {
      "type": "workspace-write",
      "writable_roots": ["/home/user/dev/dev"],
...
The Hanzo Dev harness (used by the CLI and the VS Code extension) sends such requests to MCP servers that declare the `dev/sandbox-state` capability.

## Package Contents

This package wraps the `dev-exec-mcp-server` binary and its helpers so that the shell MCP can be invoked via `npx -y @openai/codex-shell-tool-mcp`. It bundles:

- `dev-exec-mcp-server` and `dev-execve-wrapper` built for macOS (arm64, x64) and Linux (musl arm64, musl x64).
...
See [the README in the Hanzo Dev repo](https://github.com/hanzoai/dev/blob/main/code-rs/exec-server/README.md) for details.
