...
def get_config(
    ...
    logger.info(
        ...
        f'Submit an issue on https://github.com/hanzoai/build if you run into any issues.'
    )
...
def initialize_runtime(
    ...
    action = CmdRunAction(command='git checkout -b hanzo')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        obs.exit_code == 0, f'Failed to git checkout new branch hanzo: {str(obs)}'
    )
...
def complete_runtime(
    ...
    action = CmdRunAction(command='git commit -m "hanzo edits"')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        ...
        f'Failed to git commit -m "hanzo": {str(obs)}',
    )
