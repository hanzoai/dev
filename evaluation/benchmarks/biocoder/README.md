# BioCoder Evaluation with Opendev
...
In the dev branch of the Biocoder repository, we have slightly modified our original Docker image to work with the Dev environment. In the Docker image are testing scripts (`/testing/start_test_dev.py` and aux files in `/testing_files/`) to assist with evaluation. Additionally, we have installed all dependencies, including OpenJDK, mamba (with Python 3.6), and many system libraries. Notably, we have **not** packaged all repositories into the image, so they are downloaded at runtime.
...
To reproduce this image, please see the Dockerfile_Opendev in the `biocoder` repository.
...
with current Dev version, then your command would be:
