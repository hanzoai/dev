checkout_eval_branch() {
    ...
    echo "Start to checkout dev version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out Dev to version: $COMMIT_HASH"
...
get_dev_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of Dev
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    DEV_VERSION=v$(poetry run python -c "from dev import get_version; print(get_version())")
}
