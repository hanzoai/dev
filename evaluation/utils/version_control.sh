checkout_eval_branch() {
    ...
    echo "Start to checkout ide version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out IDE to version: $COMMIT_HASH"
...
get_ide_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of IDE
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    IDE_VERSION=v$(poetry run python -c "from ide import get_version; print(get_version())")
}
