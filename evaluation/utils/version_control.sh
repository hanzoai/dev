checkout_eval_branch() {
    ...
    echo "Start to checkout hanzo version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out Hanzo to version: $COMMIT_HASH"
...
get_hanzo_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of Hanzo
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    HANZO_VERSION=v$(poetry run python -c "from hanzo import get_version; print(get_version())")
}
