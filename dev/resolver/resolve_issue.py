...
from dev.core.config import AgentConfig, AppConfig, LLMConfig, SandboxConfig
...
async def resolve_issue(
    ...
    if repo_instruction is None:
        # Check for .dev_instructions file in the workspace directory
        dev_instructions_path = os.path.join(repo_dir, '.dev_instructions')
        if os.path.exists(dev_instructions_path):
            with open(dev_instructions_path, 'r') as f:  # noqa: ASYNC101
...
def main() -> None:
    ...
    if runtime_container_image is None and not my_args.is_experimental:
        runtime_container_image = (
            f'ghcr.io/hanzoai/runtime:{dev.__version__}-nikolaik'
        )
