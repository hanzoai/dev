...
from dev.resolver.interfaces.gitlab import GitlabIssueHandler
from dev.resolver.interfaces.issue import Issue
from dev.resolver.interfaces.issue_definitions import ServiceContextIssue
from dev.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from dev.resolver.patching import apply_diff, parse_patch
from dev.resolver.resolver_output import ResolverOutput
from dev.resolver.utils import (
    Platform,
    identify_token,
)
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        logger.info('Git user configured as dev')
    ...
    if not status_result.stdout.strip():
        ...
        raise RuntimeError('ERROR: Dev failed to make code changes.')
...
def send_pull_request(
    ...
    base_branch_name = f'dev-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [Dev](https://github.com/hanzoai/dev/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'Dev made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new Dev update is available, but failed to parse or summarize the changes:\n{additional_message}'
