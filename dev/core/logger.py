...
def get_file_handler(
    ...
    file_name = f'dev_{timestamp}.log'
...
dev_logger.setLevel(current_log_level)

if DEBUG:
    dev_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    dev_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    dev_logger.addHandler(json_log_handler(current_log_level))
else:
    dev_logger.addHandler(get_console_handler(current_log_level))

dev_logger.addFilter(SensitiveDataFilter(dev_logger.name))
dev_logger.propagate = False
dev_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of dev/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    dev_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    dev_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        dev_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class DevLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(self, logger=dev_logger, extra=None):
        self.logger = logger
        self.extra = extra or {}
