...
from dev.events.event import Event, EventSource, RecallType
from dev.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from dev.events.observation.empty import NullObservation
from dev.events.stream import EventStream, EventStreamSubscriber
...
from dev.utils.prompt import RepositoryInfo, RuntimeInfo

GLOBAL_MICROAGENTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(dev.__file__)),
    'microagents',
)
...
class Memory:
    ...
    def __init__(
        ...
        # from typically Dev/microagents (i.e., the PUBLIC microagents)
