...
from dev import __version__ as oh_version
from dev.core.exceptions import AgentRuntimeBuildError
from dev.core.logger import RollingLogger
from dev.core.logger import dev_logger as logger
from dev.runtime.builder.base import RuntimeBuilder
from dev.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running dev in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official dev app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside dev-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
        ...
        buildx_cmd = [
            ...
            'build',
            '--progress=plain',
            f'--build-arg=DEV_RUNTIME_VERSION={oh_version}',
            f'--build-arg=DEV_RUNTIME_BUILD_TIME={datetime.datetime.now().isoformat()}',
