...
from dev.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the Dev runtime Docker image using the docker build folder.
...
def prep_build_folder(
    ...
    dev_source_dir = Path(dev.__file__).parent
    project_root = dev_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'dev' directory (Source code)
    shutil.copytree(
        dev_source_dir,
        Path(build_folder, 'code', 'dev'),
...
def get_hash_for_lock_files(base_image: str) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(dev_source_dir, file)
        if not src.exists():
            src = Path(dev_source_dir.parent, file)
