...
from dev.events.serialization import event_to_dict, observation_from_dict
from dev.runtime.impl.action_execution.action_execution_client import (
    ActionExecutionClient,
)
from dev.runtime.impl.docker.docker_runtime import (
    APP_PORT_RANGE_1,
    APP_PORT_RANGE_2,
...
from dev.runtime.utils.command import get_action_execution_server_startup_command
from dev.utils.async_utils import call_sync_from_async
from dev.utils.tenacity_stop import stop_if_should_exit


def check_dependencies(code_repo_path: str, poetry_venvs_path: str):
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/hanzoai/dev/blob/main/Development.md to install Dev.'
    ...
    from dev.runtime.browser.browser_env import BrowserEnv
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        if self.config.workspace_base is not None:
            ...
            self._temp_workspace = tempfile.mkdtemp(
                prefix=f'dev_workspace_{sid}',
            )
            self.config.workspace_mount_path_in_sandbox = self._temp_workspace

        logger.warning(
            'Initializing LocalRuntime. WARNING: NO SANDBOX IS USED. '
            'This is an experimental feature, please report issues to https://github.com/hanzoai/dev/issues. '
            '`run_as_dev` will be ignored since the current user will be used to launch the server. '
    ...
    async def connect(self):
        ...
        code_repo_path = os.path.dirname(os.path.dirname(dev.__file__))
        env['PYTHONPATH'] = f'{code_repo_path}:$PYTHONPATH'
        env['DEV_REPO_PATH'] = code_repo_path
