...
from dev.core.config import AppConfig, SandboxConfig
from dev.core.exceptions import AgentRuntimeDisconnectedError
from dev.core.logger import dev_logger as logger
from dev.events import EventSource, EventStream, EventStreamSubscriber
...
from dev.events.serialization.action import ACTION_TYPE_TO_CLASS
from dev.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from dev.microagent import (
    BaseMicroAgent,
    load_microagents_from_dir,
)
from dev.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from dev.runtime.utils.edit import FileEditRuntimeMixin
...
class Runtime(FileEditRuntimeMixin):
    ...
    def clone_repo(
        ...
        dev_workspace_branch = f'dev-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {dev_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .dev/setup.sh if it exists in the workspace or repository."""
        setup_script = '.dev/setup.sh'
    ...
    def get_microagents_from_selected_repo(
        ...
        microagents_dir = workspace_root / '.dev' / 'microagents'
        repo_root = None
        if selected_repository:
            repo_root = workspace_root / selected_repository.split('/')[-1]
            microagents_dir = repo_root / '.dev' / 'microagents'
        self.log(
            'info',
        ...
        # Check for legacy .dev_instructions file
        obs = self.read(
            FileReadAction(path=str(workspace_root / '.dev_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.dev_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.dev_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'dev_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroAgent.load(
                    path='.dev_instructions', file_content=obs.content
                )
            )
