...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    dev_fork1.submit(Op::GetHistory).await.unwrap();
    let fork1_history = wait_for_event(&dev_fork1, |ev| {
    ...
    dev_fork2.submit(Op::GetHistory).await.unwrap();
    let fork2_history = wait_for_event(&dev_fork2, |ev| {
        matches!(ev, EventMsg::ConversationHistory(_))
    })
    .await;
    assert!(matches!(
