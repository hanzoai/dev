...
/// Writes an `auth.json` into the provided `dev_home` with the specified parameters.
...
async fn includes_session_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "dev_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut config = load_default_config_for_test(&dev_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_dev_auth());
    let NewConversation {
        conversation: codex,
        conversation_id,
        session_configured: _,
...
async fn prefers_chatgpt_token_when_config_prefers_chatgpt() {
    ...
    let auth_manager = match CodexAuth::from_dev_home(
        dev_home.path(),
        config.preferred_auth_method,
        &config.responses_originator_header,
    ) {
        Ok(Some(auth)) => dev_core::AuthManager::from_auth_for_testing(auth),
        Ok(None) => panic!("No CodexAuth found in dev_home"),
...
fn create_dummy_dev_auth() -> CodexAuth {
