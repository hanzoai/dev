...
const DEFAULT_RESPONSES_ORIGINATOR_HEADER: &str = "dev_cli_rs";
...
pub struct Config {
    ...
    pub dev_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    ...
    pub fn load_with_cli_overrides(
        ...
        // `Config` instance.
        let dev_home = find_dev_home()?;

        // Step 1: parse `config.toml` into a generic JSON value.
        let mut root_value = load_config_as_toml(&dev_home)?;
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, dev_home)
    }
}
...
pub fn load_config_as_toml(dev_home: &Path) -> std::io::Result<TomlValue> {
    let config_path = dev_home.join(CONFIG_TOML_FILE);
...
/// Use with caution.
pub fn set_project_trusted(dev_home: &Path, project_path: &Path) -> anyhow::Result<()> {
    let config_path = dev_home.join(CONFIG_TOML_FILE);
    ...
    // ensure dev_home exists
    std::fs::create_dir_all(dev_home)?;

    // create a tmp_file
    let tmp_file = NamedTempFile::new_in(dev_home)?;
...
pub fn set_tui_theme_name(dev_home: &Path, theme: ThemeName) -> anyhow::Result<()> {
    let config_path = dev_home.join(CONFIG_TOML_FILE);
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let user_instructions = Self::load_instructions(Some(&dev_home));
        ...
        // Check if we're using ChatGPT auth before moving dev_home
        let using_chatgpt_auth = Self::is_using_chatgpt_auth(&dev_home);
        ...
        let config = Self {
            ...
            // Already computed before moving dev_home
            using_chatgpt_auth,
        };
        Ok(config)
    }
    ...
    fn is_using_chatgpt_auth(dev_home: &Path) -> bool {
        use dev_protocol::mcp_protocol::AuthMode;
        use crate::CodexAuth;
        
        match CodexAuth::from_dev_home(dev_home, AuthMode::ApiKey, "dev_cli_rs") {
            Ok(Some(auth)) => auth.mode == AuthMode::ChatGPT,
            _ => false,
        }
    }
    
    fn load_instructions(dev_dir: Option<&Path>) -> Option<String> {
        let mut p = match dev_dir {
...
"#;
    ...
    impl PrecedenceTestFixture {
        ...
        fn dev_home(&self) -> PathBuf {
            self.dev_home.path().to_path_buf()
        }
    }
...
"#;
        ...
        let dev_home_temp_dir = TempDir::new().unwrap();
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                use_experimental_streamable_shell_tool: false,
                responses_originator_header: "dev_cli_rs".to_string(),
                debug: false,
    ...
    fn test_set_project_trusted_writes_explicit_tables() -> anyhow::Result<()> {
        ...
        set_project_trusted(dev_home.path(), project_dir.path())?;

        // Read back the generated config.toml and assert exact contents
        let config_path = dev_home.path().join(CONFIG_TOML_FILE);
...
"#
        ...
        std::fs::create_dir_all(dev_home.path())?;
