pub const DEFAULT_ORIGINATOR: &str = "dev_cli_rs";
...
pub fn create_client(originator: &str) -> reqwest::Client {
    ...
    let ua = get_dev_user_agent(Some(originator));
...
mod tests {
    ...
    #[test]
    fn test_get_dev_user_agent() {
        let user_agent = get_dev_user_agent(None);
        assert!(user_agent.starts_with("dev_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        let expected_ua = get_dev_user_agent(Some(originator));
    ...
    fn test_macos() {
        ...
        let user_agent = get_dev_user_agent(None);
        let re = Regex::new(
            r"^dev_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();
