...
impl RolloutRecorder {
    ...
    pub async fn list_conversations(
        ...
        get_conversations(dev_home, page_size, cursor).await
    }
...
fn create_log_file(config: &Config, session_id: Uuid) -> std::io::Result<LogFileInfo> {
    ...
    let mut dir = config.dev_home.clone();
...
impl IndexContext {
    fn new(dev_home: PathBuf, cwd: PathBuf, session_path: PathBuf, model: Option<String>) -> Self {
        Self { dev_home, cwd, session_path, model }
    }
