...
use base64::Engine;
use dev_apply_patch::ApplyPatchAction;
use dev_apply_patch::MaybeApplyPatchVerified;
use dev_apply_patch::maybe_parse_apply_patch_verified;
// unused: AuthManager
...
use dev_protocol::models::WebSearchAction;
...
reasoning: {:?}"#,
    ...
    if let Some(browser_manager) = dev_browser::global::get_browser_manager().await {
...
use dev_protocol::models::ShellToolCallParams;
    ...
    async fn run_exec_with_events<'a>(
        ...
        self.on_exec_command_begin(turn_diff_tracker, begin_ctx.clone(), seq_hint, output_index, attempt_req)
            ...
            let result = process_exec_tool_call(
                ...
                exec_args.dev_linux_sandbox_exe,
...
async fn run_turn(
    ...
    let browser_enabled = dev_browser::global::get_browser_manager().await.is_some();
...
async fn handle_web_fetch(sess: &Session, ctx: &ToolCallCtx, arguments: String) -> ResponseInputItem {
    ...
    execute_custom_tool(
        ...
        || async move {
            ...
            let dev_ua = crate::default_client::get_dev_user_agent(Some("web_fetch"));
            ...
            if !matches!(params.mode.as_deref(), Some("http")) && is_dynamic_domain {
                let browser_manager = dev_browser::global::get_or_create_browser_manager().await;
            ...
            let resp = match do_request(&params.url, &dev_ua, timeout, None).await {
                Ok(r) => r,
                Err(e) => {
                    return ResponseInputItem::FunctionCallOutput {
                        call_id: call_id_clone,
...
pub struct ExecInvokeArgs<'a> {
    ...
    pub dev_linux_sandbox_exe: &'a Option<PathBuf>,
    ...
    let output_result = sess
        .run_exec_with_events(
            ...
            ExecInvokeArgs {
                ...
                dev_linux_sandbox_exe: &sess.dev_linux_sandbox_exe,
...
async fn capture_browser_screenshot(_sess: &Session) -> Result<(PathBuf, String), String> {
    let browser_manager = dev_browser::global::get_browser_manager()
        .await
        ...
        || async move {
            ...
            match args {
                Ok(json) => {
                    ...
                    let browser_manager = {
                        let existing_global = dev_browser::global::get_browser_manager().await;
...
async fn get_browser_manager_for_session(
    _sess: &Session,
) -> Option<Arc<dev_browser::BrowserManager>> {
    // Always use the global browser manager
    dev_browser::global::get_browser_manager().await
}
            ...
            if let Some(browser_manager) = browser_manager {
                ...
                match browser_manager.stop().await {
                    Ok(_) => {
                        ...
                        dev_browser::global::clear_browser_manager().await;
                        ResponseInputItem::FunctionCallOutput {
                            call_id: call_id_clone.clone(),
                            output: FunctionCallOutputPayload {
