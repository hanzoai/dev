...
impl McpProcess {
    pub async fn new(dev_home: &Path) -> anyhow::Result<Self> {
    ...
    pub async fn send_dev_tool_call(
        &mut self,
        params: CodexToolCallParam,
    ) -> anyhow::Result<i64> {
        let dev_tool_call_params = CallToolRequestParams {
            name: "codex".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(dev_tool_call_params)?),
        )
        .await
    }
