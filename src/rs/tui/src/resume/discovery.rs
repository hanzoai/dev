...
pub fn list_sessions_for_cwd(cwd: &Path, dev_home: &Path) -> Vec<ResumeCandidate> {
    // First: try per-directory index
    if let Some(mut v) = read_dir_index(dev_home, cwd) {
...
fn read_dir_index(dev_home: &Path, cwd: &Path) -> Option<Vec<ResumeCandidate>> {
    let index_path = super_sanitize_dir_index_path(dev_home, cwd);
...
fn super_sanitize_dir_index_path(dev_home: &Path, cwd: &Path) -> PathBuf {
    ...
    let mut p = dev_home.to_path_buf();
    p.push("sessions");
    p.push("index");
    p.push("by-dir");
