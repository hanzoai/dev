...
use app::App;
use dev_core::BUILT_IN_OSS_MODEL_PROVIDER_ID;
...
pub async fn run_main(
...
) -> std::io::Result<dev_core::protocol::TokenUsage> {
    ...
    #[allow(clippy::print_stderr)]
    let config_toml = {
        let dev_home = match find_dev_home() {
            Ok(dev_home) => dev_home,
            Err(err) => {
        ...
        match load_config_as_toml_with_cli_overrides(&dev_home, cli_kv_overrides) {
    ...
    let log_dir = dev_core::config::log_dir(&config)?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("dev_core=info,dev_tui=info,dev_browser=info")
        })
    };
    ...
    if cli.oss {
        dev_ollama::ensure_oss_ready(&config)
            .await
...
) -> color_eyre::Result<dev_core::protocol::TokenUsage> {
    color_eyre::install()?;
...
pub fn get_login_status(config: &Config) -> LoginStatus {
    let dev_home = config.dev_home.clone();
    match CodexAuth::from_dev_home(&dev_home, AuthMode::ApiKey, &config.responses_originator_header) {
