...
async fn end_to_end_login_flow_persists_auth_json() {
    ...
    let dev_home = tmp.path().to_path_buf();

    let state = "test_state_123".to_string();

    // Run server in background
    let server_home = dev_home.clone();

    let opts = ServerOptions {
        dev_home: server_home,
    ...
    // Validate auth.json
    let auth_path = dev_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_dev_home_dir() {
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
    ...
    let tmp = tempdir().unwrap();
    let dev_home = tmp.path().join("missing-subdir"); // does not exist
