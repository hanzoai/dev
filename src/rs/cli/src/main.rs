...
use dev_chatgpt::apply_command::ApplyCommand;
use dev_chatgpt::apply_command::run_apply_command;
use dev_cli::LandlockCommand;
use dev_cli::SeatbeltCommand;
use dev_cli::login::run_login_status;
use dev_cli::login::run_login_with_api_key;
use dev_cli::login::run_login_with_chatgpt;
use dev_cli::login::run_logout;
use dev_cli::proto;
use dev_common::CliConfigOverrides;
use dev_exec::Cli as ExecCli;
use dev_tui::Cli as TuiCli;
...
struct OrderReplayArgs {
    /// Path to a response.json captured under ~/.dev/debug_logs/*_response.json
    response_json: std::path::PathBuf,
    /// Path to dev-tui.log (typically ~/.dev/log/dev-tui.log)
    tui_log: std::path::PathBuf,
}

fn main() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|dev_linux_sandbox_exe| async move {
        cli_main(dev_linux_sandbox_exe).await?;
        Ok(())
    })
}

async fn cli_main(dev_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match cli.subcommand {
        ...
        Some(Subcommand::Debug(debug_args)) => match debug_args.cmd {
            DebugCommand::Seatbelt(mut seatbelt_cli) => {
                ...
                dev_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
            ...
            DebugCommand::Landlock(mut landlock_cli) => {
                ...
                dev_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    dev_linux_sandbox_exe,
                )
                .await?;
        ...
        Some(Subcommand::GenerateTs(gen_cli)) => {
            dev_protocol_ts::generate_ts(&gen_cli.out_dir, gen_cli.prettier.as_deref())?;
        }
...
async fn doctor_main() -> anyhow::Result<()> {
    ...
    println!("dev version: {}", dev_version::version());
