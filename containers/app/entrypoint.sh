...
if [ "$(id -u)" -ne 0 ]; then
  echo "The IDE entrypoint.sh must run as root"
  exit 1
fi
...
if [ -z "$WORKSPACE_MOUNT_PATH" ]; then
  # This is set to /opt/workspace in the Dockerfile. But if the user isn't mounting, we want to unset it so that IDE doesn't mount at all
  unset WORKSPACE_BASE
fi

if [[ "$SANDBOX_USER_ID" -eq 0 ]]; then
  echo "Running IDE as root"
  export RUN_AS_IDE=false
  "$@"
else
  ...
  if id "enduser" &>/dev/null; then
    ...
    if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
      echo "Failed to create user enduser with id $SANDBOX_USER_ID. Moving ide user."
      incremented_id=$(($SANDBOX_USER_ID + 1))
      usermod -u $incremented_id ide
      if ! useradd -l -m -u $SANDBOX_USER_ID -s /bin/bash enduser; then
  ...
  # get the user group of /var/run/docker.sock and set ide to that group
