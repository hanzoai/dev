...
if [[ -n $GITHUB_REF_NAME ]]; then
  ...
  IDE_BUILD_VERSION=$sanitized_ref_name
...
if [[ $dry_run -eq 1 ]]; then
  ...
  jq -n \
    ...
    --arg ide_build_version "$IDE_BUILD_VERSION" \
    --arg dockerfile "$dir/Dockerfile" \
    '{
      tags: $tags,
      platform: $platform,
      build_args: [
        "IDE_BUILD_VERSION=" + $ide_build_version
      ],
...
docker buildx build \
  $args \
  --build-arg IDE_BUILD_VERSION="$IDE_BUILD_VERSION" \
