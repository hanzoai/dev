#!/usr/bin/env node
// Unified entry point for the Hanzo Node CLI.
...
// Binary names for hanzo-node
let binaryPath = path.join(__dirname, "..", "bin", `hanzo-${targetTriple}`);
...
const getCachedBinaryPath = (version) => {
  ...
  return path.join(cacheDir, `hanzo-${targetTriple}`);
};
...
const tryBootstrapBinary = async () => {
  try {
    ...
    try {
      ...
      const name = (() => {
        if (platform === "win32") return "hanzo-node-win32-x64";
        const plt = nodePlatform();
        const cpu = nodeArch();
...
if (existsSync(binaryPath)) {
  ...
  console.error(`  npm uninstall -g hanzo-node`);
  console.error(`  npm install -g hanzo-node`);
  if (isWSL()) {
    console.error("Detected WSL. Install inside WSL (Ubuntu) separately:");
    console.error("  npx -y hanzo-node@latest  (run inside WSL)");
...
if (!validation.ok) {
  ...
  if (isWSL()) {
    ...
    console.error("  npx -y hanzo-node@latest  (inside WSL)");
  }
  process.exit(1);
}
...
try {
  ...
  if (isNpx && process.stderr && process.stderr.isTTY) {
    console.error(`hanzo-node: running bundled binary -> ${binaryPath}`);
  }
} catch {}

const { spawn } = await import("child_process");

process.env.HANZO_BINARY_PATH = binaryPath;

const child = spawn(binaryPath, process.argv.slice(2), {
  stdio: "inherit",
  env: { ...process.env, HANZO_MANAGED_BY_NPM: "1", HANZO_BINARY_PATH: binaryPath },
});
...
child.on("error", (err) => {
  ...
  if (code === 'EACCES') {
    ...
    console.error(`Or reinstall the package with: npm install -g hanzo-node`);
    ...
    console.error("  npm uninstall -g hanzo-node && npm install -g hanzo-node");
    if (platform === 'win32') {
    ...
    if (isWSL()) {
      ...
      console.error("Install inside WSL and run there: npx -y hanzo-node@latest");
    }
  } else {
    console.error(err);
  }
