...
Microagents are specialized prompts that enhance IDE with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each microagent is designed to excel in a specific area, from Git operations to code review processes.

## Sources of Microagents

IDE loads microagents from two sources:

### 1. Shareable Microagents (Public)
This directory (`IDE/microagents/`) contains shareable microagents that are:
- Available to all IDE users
- Maintained in the IDE repository
...
Each repository can have its own instructions in `.ide/microagents/repo.md`. These instructions are:
...
When IDE works with a repository, it:
1. Loads repository-specific instructions from `.ide/microagents/repo.md` if present
...
You can see an example of a knowledge-based agent in [IDE's github microagent](https://github.com/hanzoai/ide/tree/main/microagents/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:
- Loaded from `.ide/microagents/repo.md`
...
You can see an example of a repo agent in [the agent for the IDE repo itself](https://github.com/hanzoai/ide/blob/main/.ide/microagents/repo.md).
...
2. **For Repository Agents**:
   ...
   - Consider using IDE to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.ide/microagents/` directory
2. Test thoroughly
3. Submit a pull request to IDE


## License

All microagents are subject to the same license as IDE. See the root LICENSE file for details.
