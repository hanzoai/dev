...
function extractResponse(output, prompt) {
    ...
    response = response.split('\n')
        .filter(line => line.trim() && !line.includes('Claude') && !line.startsWith('>'))
        .join('\n')
        .trim();
    ...
    if (url.pathname === '/v1/chat/completions' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                console.log(`[${new Date().toISOString()}] PTY Request: ${prompt.substring(0, 50)}...`);

                const response = await sendToClaude(prompt);

                console.log(`[${new Date().toISOString()}] PTY Response: ${response.substring(0, 100)}...`);
