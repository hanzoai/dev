...
const PORT = parseInt(process.env.PORT || process.argv.find(a => a.startsWith('--port='))?.split('=')[1] || '9999');
...
const CLI_BACKENDS = {
    ...
    'codex': {
        ...
        env: { OTEL_SDK_DISABLED: 'true' },
        inputMode: 'args',
        models: ['codex', 'codex-mini', 'codex-mini-latest', 'gpt-4o', 'gpt-4o-mini', 'o1', 'o3-mini'],
    },
    ...
    'vibe': {
        command: 'vibe',
        args: (prompt, model) => ['-p', prompt, '--output', 'text', '--auto-approve'],
        env: {},
        inputMode: 'args',
...
function getBackend(model) {
    ...
    // Prefix matching
    if (model.startsWith('claude')) return { ...CLI_BACKENDS['claude'], name: 'claude' };
    if (model.startsWith('codex') || model.startsWith('gpt') || model.startsWith('o1') || model.startsWith('o3')) {
        return { ...CLI_BACKENDS['codex'], name: 'codex' };
    }
    if (model.startsWith('gemini')) return { ...CLI_BACKENDS['gemini'], name: 'gemini' };
    if (model.startsWith('vibe') || model.startsWith('mistral')) return { ...CLI_BACKENDS['vibe'], name: 'vibe' };
...
function invokeCLI(prompt, model) {
    return new Promise((resolve, reject) => {
        ...
        console.log(`[${new Date().toISOString()}] Backend: ${backend.name}, Command: ${backend.command} ${args.slice(0, 2).join(' ')}...`);
        ...
        proc.on('close', (code) => {
            if (code !== 0 && !stdout) {
                reject(new Error(`${backend.name} CLI exited with code ${code}: ${stderr}`));
            } else {
    ...
    if (url.pathname === '/v1/chat/completions' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                console.log(`[${new Date().toISOString()}] Request: model=${model}, prompt=${prompt.substring(0, 50)}...`);
