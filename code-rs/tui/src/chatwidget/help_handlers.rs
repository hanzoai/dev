...
pub(super) fn handle_help_key(chat: &mut ChatWidget<'_>, key_event: KeyEvent) -> bool {
    // If no guide overlay, intercept Ctrl+H to open it.
    if chat.help.overlay.is_none() {
        if let KeyEvent { code: KeyCode::Char('h'), modifiers, .. } = key_event {
    ...
    match key_event {
        // Ctrl+H toggles (closes) the overlay
        KeyEvent { code: KeyCode::Char('h'), modifiers, .. } if modifiers.contains(KeyModifiers::CONTROL) => {
