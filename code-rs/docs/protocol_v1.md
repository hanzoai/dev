...
NOTE: The dev might not completely match this spec. There are a few minor changes that need to be made after this spec has been reviewed, which will not alter the existing TUI's functionality.

## Entities

These are entities exit on the dev backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Hanzo Dev` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Hanzo Dev`
   - The core engine of dev
...
2. `Session`
   - The `Hanzo Dev`'s current configuration and state
   - `Hanzo Dev` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Hanzo Dev` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Hanzo Dev` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Hanzo Dev`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Hanzo Dev`, as `Hanzo Dev` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Hanzo Dev` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Hanzo Dev`)
...
- `Event`
  - These are messages sent on the `EQ` (`Hanzo Dev` -> UI)
...
- `Op`
  ...
  - `Op::ExecApproval` – Approve or deny dev execution
- `EventMsg`
...
sequenceDiagram
    ...
    user->>dev: Op::ConfigureSession
    dev-->>session: create session
    dev->>user: Event::SessionConfigured
