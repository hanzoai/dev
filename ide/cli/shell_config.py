"""Shell configuration management for IDE CLI aliases.
...
"""),
    ...
    ALIAS_PATTERNS = {
        ...
        'powershell': [
            r'^\s*function\s+ide\s*\{',
            r'^\s*function\s+oh\s*\{',
        ],
    }

    def __init__(self, command: str = 'uvx --python 3.12 --from hanzo-dev hanzo-dev'):
    ...
    def aliases_exist(self, config_path: Optional[Path] = None) -> bool:
        """Check if IDE aliases already exist in the shell config.
    ...
    def add_aliases(self, config_path: Optional[Path] = None) -> bool:
        """Add IDE aliases to the shell configuration.
...
def aliases_exist_in_shell_config() -> bool:
    """Check if IDE aliases exist in the shell configuration."""
    manager = ShellConfigManager()
