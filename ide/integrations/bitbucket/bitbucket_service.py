...
class BitBucketService(BaseGitService, GitService):
    ...
    This is an extension point in IDE that allows applications to customize Bitbucket
    integration behavior. Applications can substitute their own implementation by:
    1. Creating a class that inherits from GitService
    2. Implementing all required methods
    3. Setting server_config.bitbucket_service_class to the fully qualified name of the class

    The class is instantiated via get_impl() in ide.server.shared.py.
    """
...
bitbucket_service_cls = os.environ.get(
    'IDE_BITBUCKET_SERVICE_CLS',
    'ide.integrations.bitbucket.bitbucket_service.BitBucketService',
)
