...
The Hanzo Dev CLI supports a set of slash commands you can type at the start of the
...
- `/quit`: exit Hanzo Dev.
- `/logout`: log out of Hanzo Dev.
- `/login`: manage Hanzo Dev sign-ins (select, add, or disconnect accounts).
...
- `/undo`: open a snapshot picker so you can restore workspace files to a
  previous Hanzo Dev snapshot and optionally rewind the conversation to that point.
...
- `/push`: tell Hanzo Dev to commit, push, and monitor workflows with guarded
...
- `/review [focus]`: without arguments, opens a review picker so you can audit
  ...
  from `/settings review` when you want Hanzo Dev to rerun fixes and follow-up
  checks automatically.
- `/cloud`: browse Hanzo Dev Cloud tasks, view details, apply patches, and create
...
These commands expand into full prompts (generated by `dev-core`) and
...
- `/dev <task>`: perform a coding task (multiple agents). Prompt‑expanding.
...
- Prompt formatting for `/plan`, `/solve`, and `/dev` lives in
  ...
  `dev-gpt-5.1-dev-mini`, `claude-sonnet-4.5`, `gemini-3-flash`,
  `claude-haiku-4.5`, and `qwen-3-coder` (with
  `cloud-gpt-5.1-dev-max` gated by `CODE_ENABLE_CLOUD_AGENT_MODEL`). (`gemini`
