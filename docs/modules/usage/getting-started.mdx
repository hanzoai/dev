...
So you've [run Dev](./installation) and have
[set up your LLM](./installation#setup). Now what?

Dev can help you tackle a wide variety of engineering tasks. But the technology
...
Dev can also do a great job adding new code to an existing code base.

For example, you can ask Dev to add a new GitHub action to your project
which lints your code. Dev may take a peek at your codebase to see what language
...
Some tasks might require a bit more context. While Dev can use `ls` and `grep`
...
Dev does great at refactoring existing code, especially in small chunks.
...
Dev can also help you track down and fix bugs in your code. But as any
developer knows, bug fixing can be extremely tricky, and often Dev will need more context.
It helps if you've diagnosed the bug, but want Dev to figure out the logic.
...
Dev is capable of helping out on just about any coding task but it takes some practice
...
See [Prompting Best Practices](./prompting/prompting-best-practices) for more tips on how to get the most out of Dev.
