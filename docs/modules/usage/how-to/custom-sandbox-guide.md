...
The default Dev sandbox (`python-nodejs:python3.12-nodejs22`
...
For example, if you want Dev to have `ruby` installed, you could create a `Dockerfile` with the following content:
...
When running Dev using [the docker command](/modules/usage/installation#start-the-app), replace
...
First, ensure you can run Dev by following the instructions in [Development.md](https://github.com/hanzoai/dev/blob/main/Development.md).

### Specify the Base Sandbox Image

In the `config.toml` file within the Dev directory, set the `base_container_image` to the image you want to use.
...
Run Dev by running ```make run``` in the top level directory.
