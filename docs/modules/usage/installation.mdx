...
A system with a modern processor and a minimum of **4GB RAM** is recommended to run Dev.
...
You'll find Dev running at http://localhost:3000!
...
Upon launching Dev, you'll see a Settings page. You **must** select an `LLM Provider` and `LLM Model` and enter a corresponding `API Key`.
...
Dev requires an API key to access most language models. Here's how to get an API key from the recommended providers:
...
Now you're ready to [get started with Dev](./getting-started).

## Versions

The [docker command above](./installation#start-the-app) pulls the most recent stable release of Dev. You have other options as well:
- For a specific release, replace $VERSION in `dev:$VERSION` and `runtime:$VERSION`, with the version number.
We use SemVer so `0.9` will automatically point to the latest `0.9.x` release, and `0` will point to the latest `0.x.x` release.
- For the most up-to-date development version, replace $VERSION in `dev:$VERSION` and `runtime:$VERSION`, with `main`.
...
For the development workflow, see [Development.md](https://github.com/hanzoai/dev/blob/main/Development.md).

Are you having trouble? Check out our [Troubleshooting Guide](https://docs.hanzo.ai/modules/usage/troubleshooting).
