...
:::warning
When using a Local LLM, Hanzo may have limited functionality.
:::
...
## Run Hanzo with Docker

### Start Hanzo
Use the instructions [here](../getting-started) to start Hanzo using Docker.
...
Done! Now you can start Hanzo by: `make run`. You now should be able to connect to `http://localhost:3000/`

### Configure the Web Application

In the Hanzo UI, click on the Settings wheel in the bottom-left corner.
...
The default configuration for ollama in WSL only serves localhost. This means you can't reach it from a docker container. eg. it wont work with Hanzo. First let's test that ollama is running correctly.
...
```bash
docker ps # get list of running docker containers, for most accurate test choose the Hanzo sandbox container.
...
Done! Now you can start Hanzo by: `make run` without Docker. You now should be able to connect to `http://localhost:3000/`
