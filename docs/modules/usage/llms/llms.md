...
Hanzo can connect to any LLM supported by LiteLLM. However, it requires a powerful model to work.
...
Based on these findings and community feedback, the following models have been verified to work reasonably well with Hanzo:
...
:::warning
Hanzo will issue many prompts to the LLM you configure. Most of these LLMs cost money, so be sure to set spending
limits and monitor usage.
:::

If you have successfully run Hanzo with specific LLMs not in the list, please add them to the verified list. We
...
wait times between messages, poor responses, or errors about malformed JSON. Hanzo can only be as powerful as the
models driving it. However, if you do find ones that work, please add them to the verified list above.
:::

## LLM Configuration

The following can be set in the Hanzo UI through the Settings:
...
We have a few guides for running Hanzo with specific model providers:
...
LLM providers typically have rate limits, sometimes very low, and may require retries. Hanzo will automatically
...
If you are running Hanzo in development mode, you can also set these options in the `config.toml` file:
