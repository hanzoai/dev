...
:::tip
Dev only supports Windows via WSL. Please be sure to run all commands inside your WSL terminal.
:::
...
When running Dev, the following error is seen:
```
...
* Stop any containers where the name has the prefix `dev-runtime-` :
  `docker ps --filter name=dev-runtime- --filter status=running -aq | xargs docker stop`
* Remove any containers where the name has the prefix `dev-runtime-` :
  `docker rmi $(docker images --filter name=dev-runtime- -q --no-trunc)`
* Stop and Remove any containers / images where the name has the prefix `dev-runtime-`
