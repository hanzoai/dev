...
- `code-rs/tui/src/bottom_pane/chat_composer.rs`
...
- **Persistent history** (cross-session, fetched from `$CODE_HOME/history.jsonl`): text-only. It
...
(`code-rs/tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`code-rs/core/src/slash_commands.rs` and reused by both the composer and the command
