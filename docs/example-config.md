...
#
# This file lists all keys Dev reads from config.toml, their default values,
...
# Primary model. Defaults follow the provider: "zen5-coder" on "hanzo".
model = "zen5-coder"

# Model used by the /review feature (code reviews). Same default as `model`.
review_model = "zen5-coder"

# Provider id selected from [model_providers]. Default: "hanzo".
model_provider = "hanzo"

# Optional manual model metadata. When unset, Dev auto-detects from model.
...
# Reasoning effort: minimal | low | medium | high | xhigh (default: medium)
...
# Example: notify = ["notify-send", "Dev"]
# notify = [ ]

# In-product notices (mostly set automatically by Dev).
[notice]
...
# Force login mechanism when Dev would normally auto-select. Default: unset.
...
# Built-ins include:
# - hanzo (default; Responses API at api.hanzo.ai/v1, bearer HANZO_USER_KEY)
