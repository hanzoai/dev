...
- Location: Put files in `$HANZO_HOME/prompts/` (defaults to `~/.hanzo/prompts/`; Hanzo Dev also reads legacy `~/.code/prompts/` or `~/.codex/prompts/`).
...
- How to use:
  - Start a new session (Hanzo Dev loads custom prompts on session start).
...
- Notes:
  ...
  - New or changed files are discovered on session start. If you add a new prompt while Hanzo Dev is running, start a new session to pick it up.
