...
- Location: Put files in `$CODE_HOME/prompts/` (defaults to `~/.code/prompts/`; Dev also reads legacy `~/.codex/prompts/`).
...
- How to use:
  - Start a new session (Dev loads custom prompts on session start).
...
- Notes:
  ...
  - New or changed files are discovered on session start. If you add a new prompt while Dev is running, start a new session to pick it up.
