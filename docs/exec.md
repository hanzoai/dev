...
In non-interactive mode, Dev does not ask for command or edit approvals. By default it runs in `read-only` mode, so it cannot edit files or run commands that require network access.

Use `dev exec --full-auto` to allow file edits. Use `dev exec --sandbox danger-full-access` to allow edits and networked commands.

### Default output mode

By default, Dev streams its activity to stderr and only writes the final message from the agent to stdout. This makes it easier to pipe `dev exec` into another tool without extra filtering.
...
{"project_name":"Hanzo Dev","programming_languages":["Rust","TypeScript","Shell"]}
```
...
Dev requires a Git repository to avoid destructive changes. To disable this check, use `dev exec --skip-git-repo-check`.
...
Only the conversation context is preserved; you must still provide flags to customize Dev behavior.

```shell
dev exec --model zen5-coder --json "Review the change, look for use-after-free issues"
dev exec --model enso --json resume --last "Fix use-after-free issues"
```
