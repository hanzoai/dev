...
É recomendado um sistema com um processador moderno e um mínimo de **4GB de RAM** para executar o Dev.
...
A maneira mais fácil de executar o Dev é no Docker.
...
Você encontrará o Dev em execução em http://localhost:3000!

Você também pode [conectar o Dev ao seu sistema de arquivos local](https://docs.hanzo.ai/modules/usage/runtimes#connecting-to-your-filesystem),
executar o Dev em um [modo headless programável](https://docs.hanzo.ai/modules/usage/how-to/headless-mode),
interagir com ele através de uma [CLI amigável](https://docs.hanzo.ai/modules/usage/how-to/cli-mode),
ou executá-lo em issues marcadas com [uma ação do GitHub](https://docs.hanzo.ai/modules/usage/how-to/github-action).

## Configuração

Ao iniciar o Dev, você verá uma página de Configurações. Você **deve** selecionar um `LLM Provider` e `LLM Model` e inserir uma `API Key` correspondente.
...
Agora você está pronto para [começar com o Dev](./getting-started).

## Versões

O [comando docker acima](./installation#start-the-app) baixa a versão estável mais recente do Dev. Você também tem outras opções:
- Para uma versão específica, substitua $VERSION em `dev:$VERSION` e `runtime:$VERSION`, com o número da versão.
...
- Para a versão de desenvolvimento mais atualizada, substitua $VERSION em `dev:$VERSION` e `runtime:$VERSION`, por `main`.
...
Para o fluxo de trabalho de desenvolvimento, consulte [Development.md](https://github.com/hanzoai/dev/blob/main/Development.md).

Está tendo problemas? Confira nosso [Guia de Solução de Problemas](https://docs.hanzo.ai/modules/usage/troubleshooting).
