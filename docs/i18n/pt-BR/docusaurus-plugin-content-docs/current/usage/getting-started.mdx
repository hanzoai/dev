...
Então você [executou o Dev](./installation) e
[configurou seu LLM](./installation#setup). E agora?

O Dev pode ajudá-lo a lidar com uma ampla variedade de tarefas de engenharia. Mas a tecnologia
...
O Dev também pode fazer um ótimo trabalho adicionando novo código a uma base de código existente.

Por exemplo, você pode pedir ao Dev para adicionar uma nova ação do GitHub ao seu projeto
que faça lint do seu código. O Dev pode dar uma olhada na sua base de código para ver qual linguagem
...
Algumas tarefas podem exigir um pouco mais de contexto. Embora o Dev possa usar `ls` e `grep`
...
O Dev é ótimo em refatorar código existente, especialmente em pequenos pedaços.
...
O Dev também pode ajudá-lo a rastrear e corrigir bugs em seu código. Mas como qualquer
desenvolvedor sabe, a correção de bugs pode ser extremamente complicada e, muitas vezes, o Dev precisará de mais contexto.
Ajuda se você diagnosticou o bug, mas quer que o Dev descubra a lógica.
...
O Dev é capaz de ajudar em praticamente qualquer tarefa de codificação, mas é necessária alguma prática
...
Veja [Melhores Práticas de Prompt](./prompting/prompting-best-practices) para mais dicas sobre como obter o máximo do Dev.
