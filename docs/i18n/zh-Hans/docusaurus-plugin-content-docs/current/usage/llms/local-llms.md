...
### 启动 Dev
使用[此处](../getting-started)的说明使用 Docker 启动 Dev。
...
在运行 `dev` 时，你需要在 Dev UI 的设置中设置以下内容：
...
使用 [Development.md](https://github.com/hanzoai/dev/blob/main/Development.md) 中的说明构建 Dev。
...
完成！现在你可以通过 `make run` 启动 Dev。你现在应该能够连接到 `http://localhost:3000/`

### 配置 Web 应用程序

在 Dev UI 中，点击左下角的设置齿轮。
...
WSL 中 ollama 的默认配置只服务于 localhost。这意味着你无法从 docker 容器中访问它。例如，它不能与 Dev 一起工作。首先让我们测试 ollama 是否正确运行。
...
```bash
docker ps # 获取正在运行的 docker 容器列表，为了最准确的测试，选择 Dev 沙盒容器。
...
完成！现在你可以通过 `make run` 启动 Dev，无需 Docker。你现在应该能够连接到 `http://localhost:3000/`
