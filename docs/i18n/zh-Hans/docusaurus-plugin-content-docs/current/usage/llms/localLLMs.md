...
使用[此处](../intro)的说明通过 Docker 启动 Dev。
...
完成！现在您可以通过 `make run` 启动 Dev 而无需 Docker。现在您应该可以连接到 `http://localhost:3000/`
...
WSL 中 ollama 的默认配置仅为 localhost 提供服务。这意味着您无法从 docker 容器中访问它。比如，它不会与 Dev 一起工作。首先让我们测试 ollama 是否正常运行。
...
```bash
docker ps # 获取正在运行的 docker 容器列表，最准确的测试选择 Dev sandbox 容器。
