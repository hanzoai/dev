...
本指南概述了如何将您自己的评估基准集成到 Hanzo 框架中。

## 设置环境和 LLM 配置

请按照[此处](https://github.com/hanzoai/build/blob/main/Development.md)的说明设置您的本地开发环境。
开发模式下的 Hanzo 使用 `config.toml` 来跟踪大多数配置。
...
Hanzo 的主要入口点在 `hanzo/core/main.py` 中。以下是它的简化工作流程：
...
`run_controller()` 函数是 Hanzo 执行的核心。它管理代理、运行时和任务之间的交互，处理用户输入模拟和事件处理等事项。


## 入门最简单的方法：探索现有基准

我们鼓励您查看我们仓库的 [`evaluation/benchmarks/` 目录](https://github.com/hanzoai/build/blob/main/evaluation/benchmarks)中提供的各种评估基准。
...
通过遵循此结构，您可以在 Hanzo 框架内为您的基准创建强大的评估工作流。


## 理解 `user_response_fn`

`user_response_fn` 是 Hanzo 评估工作流中的关键组件。它模拟用户与代理的交互，允许在评估过程中自动响应。当您想要为代理的查询或操作提供一致的、预定义的响应时，此函数特别有用。
