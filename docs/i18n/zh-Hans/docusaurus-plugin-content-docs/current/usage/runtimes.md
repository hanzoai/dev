...
运行时是 Hanzo 代理可以编辑文件和运行命令的环境。

默认情况下，Hanzo 使用基于 Docker 的运行时，在您的本地计算机上运行。这意味着您只需要为使用的 LLM 付费，并且您的代码只会发送到 LLM。

我们还支持"远程"运行时，通常由第三方管理。它们可以使设置更简单、更具可扩展性，特别是当您并行运行多个 Hanzo 对话时（例如进行评估）。

## Docker 运行时
这是启动 Hanzo 时使用的默认运行时。您可能会注意到传递给 `docker run` 的一些标志使这成为可能：
...
# 将 $WORKSPACE_BASE 设置为 /home/<username>/Hanzo
#
# Windows 上的 WSL 示例
# export WORKSPACE_BASE=/mnt/c/dev/Hanzo
# 将 $WORKSPACE_BASE 设置为 C:\dev\Hanzo
```
...
请小心！没有任何措施可以阻止 Hanzo 代理删除或修改挂载到其工作区的任何文件。
...
## Modal 运行时
我们在 [Modal](https://modal.com/) 的合作伙伴也为 Hanzo 提供了一个运行时。

要使用 Modal 运行时，请创建一个帐户，然后[创建一个 API 密钥](https://modal.com/settings)。

然后，您需要在启动 Hanzo 时设置以下环境变量：
```bash
docker run # ...
    -e RUNTIME=modal \
