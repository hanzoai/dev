...
你也可以在可脚本化的[无头模式](https://docs.hanzo.ai/modules/usage/how-to/headless-mode)下运行 Dev，作为[交互式 CLI](https://docs.hanzo.ai/modules/usage/how-to/cli-mode)，或使用 [Dev GitHub Action](https://docs.hanzo.ai/modules/usage/how-to/github-action)。

## 设置

运行上述命令后，你可以在 [http://localhost:3000](http://localhost:3000) 找到正在运行的 Dev。

启动 Dev 后，你会看到一个设置模态框。你**必须**选择一个 `LLM Provider` 和 `LLM Model`，并输入相应的 `API Key`。
...
上述命令拉取最新的 Dev 稳定版本。你还有其他选择：
- 对于特定版本，使用 `docker.hanzo.ai/hanzoai/dev:$VERSION`，将 $VERSION 替换为版本号。
- 我们使用语义化版本，并发布主要版本、次要版本和补丁标签。因此，`0.9` 将自动指向最新的 `0.9.x` 版本，而 `0` 将指向最新的 `0.x.x` 版本。
- 对于最新的开发版本，你可以使用 `docker.hanzo.ai/hanzoai/dev:main`。此版本不稳定，仅建议用于测试或开发目的。
