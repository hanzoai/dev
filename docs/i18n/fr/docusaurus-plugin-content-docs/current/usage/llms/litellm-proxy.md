...
Dev prend en charge l'utilisation du [proxy LiteLLM](https://docs.litellm.ai/docs/proxy/quick_start) pour accéder à divers fournisseurs de LLM.

## Configuration

Pour utiliser le proxy LiteLLM avec Dev, vous devez :

1. Configurer un serveur proxy LiteLLM (voir la [documentation LiteLLM](https://docs.litellm.ai/docs/proxy/quick_start))
2. Lors de l'exécution d'Dev, vous devrez définir les éléments suivants dans l'interface utilisateur d'Dev via les paramètres :
...
Les modèles pris en charge dépendent de la configuration de votre proxy LiteLLM. Dev prend en charge tous les modèles que votre proxy LiteLLM est configuré pour gérer.
