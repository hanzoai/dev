...
:::warning
Lors de l'utilisation d'un LLM local, Hanzo peut avoir des fonctionnalités limitées.
:::
...
## Exécuter Hanzo avec Docker

### Démarrer Hanzo
Utilisez les instructions [ici](../getting-started) pour démarrer Hanzo en utilisant Docker.
...
Lors de l'exécution d'`hanzo`, vous devrez définir les éléments suivants dans l'interface utilisateur d'Hanzo via les paramètres :
...
## Exécuter Hanzo en mode développement
...
Terminé ! Vous pouvez maintenant démarrer Hanzo avec : `make run`. Vous devriez maintenant pouvoir vous connecter à `http://localhost:3000/`

### Configurer l'application Web

Dans l'interface utilisateur d'Hanzo, cliquez sur la roue des paramètres dans le coin inférieur gauche.
...
La configuration par défaut d'ollama dans WSL ne sert que localhost. Cela signifie que vous ne pouvez pas y accéder depuis un conteneur docker. Par ex. cela ne fonctionnera pas avec Hanzo. Testons d'abord qu'ollama fonctionne correctement.
...
```bash
docker ps # obtenir la liste des conteneurs docker en cours d'exécution, pour un test plus précis choisissez le conteneur sandbox Hanzo.
...
Terminé ! Vous pouvez maintenant démarrer Hanzo avec : `make run` sans Docker. Vous devriez maintenant pouvoir vous connecter à `http://localhost:3000/`
