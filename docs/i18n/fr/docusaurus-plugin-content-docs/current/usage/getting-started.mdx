...
Vous avez donc [installé Hanzo](./installation) et avez
[configuré votre LLM](./installation#setup). Et maintenant ?

Hanzo peut vous aider à aborder une grande variété de tâches d'ingénierie. Mais la technologie
...
Hanzo peut également faire un excellent travail en ajoutant du nouveau code à une base de code existante.

Par exemple, vous pouvez demander à Hanzo d'ajouter une nouvelle action GitHub à votre projet
qui analyse votre code. Hanzo peut jeter un coup d'œil à votre base de code pour voir quel langage
...
Certaines tâches peuvent nécessiter un peu plus de contexte. Bien qu'Hanzo puisse utiliser `ls` et `grep`
...
Hanzo est excellent pour refactoriser du code existant, surtout par petits morceaux.
...
Hanzo peut également vous aider à traquer et corriger des bugs dans votre code. Mais, comme tout
développeur le sait, la correction de bugs peut être extrêmement délicate, et souvent Hanzo aura besoin de plus de contexte.
Cela aide si vous avez diagnostiqué le bug, mais que vous voulez qu'Hanzo comprenne la logique.
...
Hanzo est capable d'aider sur à peu près n'importe quelle tâche de codage. Mais il faut de la pratique
...
Voir [Bonnes pratiques de prompting](./prompting/prompting-best-practices) pour plus de conseils sur la façon de tirer le meilleur parti d'Hanzo.
