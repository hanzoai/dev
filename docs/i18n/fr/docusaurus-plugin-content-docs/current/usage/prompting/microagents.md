...
Dev utilise des micro-agents spécialisés pour gérer efficacement des tâches et des contextes spécifiques. Ces micro-agents sont de petits composants ciblés qui fournissent un comportement et des connaissances spécialisés pour des scénarios particuliers.

## Aperçu

Les micro-agents sont définis dans des fichiers markdown sous le répertoire `dev/agenthub/codeact_agent/micro/`. Chaque micro-agent est configuré avec :
...
Les micro-agents sont automatiquement intégrés dans le workflow d'Dev. Ils :
...
Pour contribuer un nouveau micro-agent à Dev, suivez ces directives :
...
Créez un nouveau fichier markdown dans `dev/agenthub/codeact_agent/micro/` avec un nom descriptif (par ex., `docker.md` pour un agent axé sur Docker).
...
N'oubliez pas que les micro-agents sont un moyen puissant d'étendre les capacités d'Dev dans des domaines spécifiques. Des agents bien conçus peuvent améliorer significativement la capacité du système à gérer des tâches spécialisées.
