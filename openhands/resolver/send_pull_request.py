...
from hanzo.resolver.interfaces.github import GithubIssueHandler
from hanzo.resolver.interfaces.gitlab import GitlabIssueHandler
from hanzo.resolver.interfaces.issue import Issue
from hanzo.resolver.interfaces.issue_definitions import ServiceContextIssue
from hanzo.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from hanzo.resolver.patching import apply_diff, parse_patch
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "hanzo" && '
            f'git -C {repo_dir} config user.email "hanzo@hanzo.ai" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        print('Git user configured as hanzo')
    ...
    if not status_result.stdout.strip():
        ...
        raise RuntimeError('ERROR: Hanzo failed to make code changes.')
...
def send_pull_request(
    ...
    base_branch_name = f'hanzo-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [Hanzo](https://github.com/hanzoai/Hanzo/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'Hanzo made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new Hanzo update is available, but failed to parse or summarize the changes:\n{additional_message}'
