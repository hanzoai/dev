...
For example, `hanzo/agenthub/codeact_agent`, etc.
...
The `state` represents the running state of an agent in the Hanzo system. The class handles saving and restoring the agent session. It is serialized in a pickle.
...
Hanzo is a multi-agentic system. Agents can delegate tasks to other agents, whether
...
A `task` is an end-to-end conversation between Hanzo (the whole system) and the user,
...
Hanzo repo has`. Let's assume the default agent is CodeActAgent.
